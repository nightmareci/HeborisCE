...
cmake "$SOURCE_DIRECTORY" -B build -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=/usr -DPACKAGE_TYPE=Installable || exit 1
...
"./$LINUXDEPLOY" --appdir AppDir --output appimage --icon-file "$SOURCE_DIRECTORY/pkg/linux/$NAME.png" --icon-filename "$NAME" --desktop-file "$BUILD_DIRECTORY/build/$NAME.desktop" || exit 1
