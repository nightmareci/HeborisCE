...
//	statusc[player * 10 + 4] = 1; //横先行入力は常に可能
	...
	if(!statusc[player * 10 + 2]) {
		...
		if((statusc[player * 10 + 6] != 0) && (repversw >= 59))
			...
			move = (getPushState(player, INPUT_BUTTON_B) || rolling) - (getPushState(player, INPUT_BUTTON_A));
				    // for old style, force negative

		if((getPushState(player, INPUT_BUTTON_C) != 0) && (statusc[player * 10 + 6] == 0)) move = 2;
		// different for DRS
		...
		if((move != 0) && (isrotatelock[player] == 0)) {
			...
			if(judgeBlock(player, bx[player], by[player], k, bak) == 0) {
				...
				if((move==1)||(move==2)) hosei_type=1 - 0*(getPressState(player, INPUT_BUTTON_A));	// R
				else if(move==-1) hosei_type=0 + 1*(getPressState(player, INPUT_BUTTON_B));			// L
...
//		statusc[player * 10 + 4] = 1; 				 // 最初の1フレームは移動させないゾ
		...
		if((judgeBlock(player, bx[player], by[player] + 1, blk[player], rt[player]) != 0)) {
			...
			if((bk[player] > lockT) || (bk_D[player] > 30)) {
				...
				// 下入れ制限#1.60c7f9
				if((getPressState(player, INPUT_BUTTON_DOWN)) && (downtype) && (!move || nanamedown)) down_flag[player] = 1;
...
//		UpLineBlockJudge(player);
		...
		if((don) && (heboGB[player]==0)) PlaySE(WAVE_SE_KON);
