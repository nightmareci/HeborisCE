...
static const SConfig DefaultConfig = {
	...
	.bgm = 1,
	.bgmvolume = 100,
	.wavebgm = APP_SOUND_BITS_SIMPLE | APP_SOUND_FORMAT_OGG,	//BGMの選択
...
void SetDefaultConfig()
{
	...
	cfgbuf[61] =
		...
		(DefaultConfig.wavebgm & APP_SOUND_BITS_MASK);
	...
	for (int32_t pl = 0; pl < 2; pl++) {
		for (int32_t key = 0; key < 10; key++) {
			...
			const APP_JoyKey *pljoy = &DefaultConfig.joyKeyAssign[pl * 10 + key];
	...
	for (int32_t pl = 0; pl < 2; pl++) {
		...
		for (int32_t key = 0; key < 8; key++) {
			const APP_ConKey *plcon = &DefaultConfig.conKeyAssign[pl * 8 + key];
