...
void statAMove(int32_t player, int32_t kickm, int32_t kickr) {
	...
	if(!statusc[player * 10 + 2]) {
		...
		if((statusc[player * 10 + 6] != 0) && (repversw >= 59))
			...
			move = (getPushState(player, INPUT_BUTTON_B) || rolling) - (getPushState(player, INPUT_BUTTON_A) || getPushState(player, INPUT_BUTTON_C));
...
//		statusc[player * 10 + 4] = 1; 				 // 最初の1フレームは移動させないゾ
		...
		// correct gravity for 1g and higher
		int one_g = 60;
		if (repversw > 65) // new replay version
			one_g--;
		...
		if( (don) && (world_drop_snd >= 1) && (heboGB[player]==0) ) PlaySE(WAVE_SE_KON);
