...
#ifdef INPUT_ENABLE_KEYBOARD
// キーボード設定
SDL_Scancode keyAssign[INPUT_BUTTON_COUNT * 2] =
{
...
INPUT_JoyKey joyKeyAssign[INPUT_BUTTON_COUNT * 2];	// ジョイスティックボタン割り当て
#endif

#ifdef INPUT_ENABLE_GAME_CONTROLLER
INPUT_ConKey conKeyAssign[8 * 2];
#endif
...
int32_t SaveConfig(void) {
	...
	for (int32_t pl = 0; pl < 2; pl++) {
		for (int32_t key = 0; key < 10; key++) {
			...
			INPUT_JoyKey *pljoy = &joyKeyAssign[pl * 10 + key];
	...
	for (int32_t pl = 0; pl < 2; pl++) {
		...
		for (int32_t key = 0; key < 8; key++) {
			const INPUT_ConKey* plcon = &conKeyAssign[pl * 8 + key];
	...
	FILESYSTEM_WriteFile32("config/data/CONFIG.SAV", cfgbuf, SDL_arraysize(cfgbuf));
...
int32_t LoadConfig(void) {
	int32_t cfgbuf[CFG_LENGTH];
	SDL_memset(cfgbuf, 0, sizeof(cfgbuf));
	FILESYSTEM_ReadFile32("config/data/CONFIG.SAV", cfgbuf, SDL_arraysize(cfgbuf), 0);
	...
	#ifdef VIDEO_ENABLE_ALL_SETTINGS
	screenIndex = cfgbuf[5];

	#else
	screenMode &= VIDEO_SCREEN_MODE_DETAIL_LEVEL | VIDEO_SCREEN_MODE_SCALE_MODE | VIDEO_SCREEN_MODE_RENDER_LEVEL;
	#ifdef SDL_PLATFORM_VITA
	screenMode &= ~VIDEO_SCREEN_MODE_RENDER_LEVEL;
	#endif
	screenMode |= VIDEO_SCREEN_MODE_BASE;
	screenIndex = 0;
	#endif
	...
	for (int32_t pl = 0; pl < 2; pl++) {
		for (int32_t key = 0; key < 10; key++) {
			...
			pljoy->type = (INPUT_JoyKeyType)plbuf[5];
			switch (pljoy->type) {
			case INPUT_JOY_KEY_AXIS:
			case INPUT_JOY_KEY_HAT:
	...
	for (int32_t pl = 0; pl < 2; pl++) {
		int32_t* plbuf = &conkeybuf[pl * (1 + 2 * 8) + 1];
		for (int32_t key = 0; key < 8; key++) {
			INPUT_ConKey* plcon = &conKeyAssign[pl * 8 + key];
			plcon->type = (INPUT_ConKeyType)plbuf[key * 2 + 0];
...
void ConfigMenu() {
	...
	if (init) {
		...
		for (int32_t pl = 0; pl < 2; pl++) {
			...
			INPUT_JoyKey *pljoy = &joyKeyAssign[pl * 10];
			plbuf[0] = pljoy->player;
			for (int32_t i = 0; i < 4; i++) {
				plbuf[1 + i] = pljoy->guid.data[i];
			}
	...
	if(background == 0) {
		for(int32_t i = 0; i <= 4; i++) {
			if(getDrawRate() == 1)
				VIDEO_DrawPlaneRect(PLANE_HEBOFLB1, 96 * i - (n % 96) / 3, 0, 0, 0, 96, 240);
			else
				VIDEO_DrawPlaneRect(PLANE_HEBOFLB1, 192 * i - (n % 32), 0, 0, 0, 192, 480);
		}
	...
	if(status[0] == 0) {
		...
		printMenuButton(2, 28, INPUT_BUTTON_A, 0);
		printMenuButton(17, 28, INPUT_BUTTON_B, 0);
		...
		for(int32_t pl = 0; pl < 2; pl++) {
			...
			if((mpc2[0] == 1) || ((mpc2[0] > tame3) && (mpc2[0] % tame4 == 0)))
				if(getPressState(pl, INPUT_BUTTON_UP)) m--;

			// ↓
			if((mpc2[0] == 1) || ((mpc2[0] > tame3) && (mpc2[0] % tame4 == 0)))
				if(getPressState(pl, INPUT_BUTTON_DOWN)) m++;
			padRepeat(pl);
			if(m) {
				PlaySE(WAVE_SE_MOVE);
			...
			if(getPushState(pl, INPUT_BUTTON_D)) {
				SDL_assert(pages > 0);
				status[0] = (status[0] + 1 + pages)%pages;
				statusc[0] = 0;
				statusc[1] = 1;
			} else {
				m = getPushState(pl, INPUT_BUTTON_RIGHT) - getPushState(pl, INPUT_BUTTON_LEFT);
			...
			if(getPushState(pl, INPUT_BUTTON_A)) {	// A:保存&再起動
			...
			if(getPushState(pl, INPUT_BUTTON_B)) {	// B:設定破棄&タイトル画面に戻る
		...
		printMenuButton(2, 28, INPUT_BUTTON_A, 0);
		printMenuButton(4, 28, INPUT_BUTTON_B, 0);
		...
		for(int32_t pl = 0; pl < 2; pl++) {
			...
			if(getPushState(pl, INPUT_BUTTON_A) || getPushState(pl, INPUT_BUTTON_B)) {	// A&B:mainに戻る
				PlaySE(WAVE_SE_KACHI);
				status[0] = 0;
				statusc[0] = 0;
				statusc[1] = 1;
		...
		if(statusc[2] == optionIndex++) {
			...
			printMenuButton(2, 28, INPUT_BUTTON_A, 0);
			printMenuButton(12, 28, INPUT_BUTTON_B, 0);
			...
			for(int32_t pl = 0; pl < 2; pl++) {
				...
				if(getPushState(pl, INPUT_BUTTON_A) && (statusc[0] != 0)) {
				...
				if(getPushState(pl, INPUT_BUTTON_B)) {
					PlaySE(WAVE_SE_KACHI);
					status[0] = 0;
					statusc[0] = 0;
					statusc[1] = 1;
			...
			// keyboard 1p&2p
			bool cancel = false;
			#if defined(INPUT_ENABLE_JOYSTICK) || defined(INPUT_ENABLE_GAME_CONTROLLER) || defined(INPUT_ENABLE_LINUX_GPIO)
			switch (INPUT_GetLastDeviceType()) {
			#ifdef INPUT_ENABLE_JOYSTICK
			case INPUT_DEVICE_TYPE_JOYSTICK:
				if (statusc[0] < 10 && INPUT_IsPushJoyKey(&joyKeyAssign[INPUT_BUTTON_B])) {
					PlaySE(WAVE_SE_MOVE);
					statusc[0] = 0;
					statusc[2] = 0;
					cancel = true;
			...
			case INPUT_DEVICE_TYPE_LINUX_GPIO:
				if (statusc[0] < 10 && INPUT_IsPushGPIO(INPUT_BUTTON_B)) {
					PlaySE(WAVE_SE_MOVE);
					statusc[0] = 0;
					statusc[2] = 0;
					cancel = true;
			...
			case INPUT_DEVICE_TYPE_NINTENDO:
				if (statusc[0] < 10 && IsPushMenu(0, INPUT_BUTTON_B, INPUT_GetLastDeviceType())) {
					PlaySE(WAVE_SE_MOVE);
					statusc[0] = 0;
					statusc[2] = 0;
					cancel = true;
			...
			if (!cancel) {
				...
				printMenuButton(3, 6, INPUT_BUTTON_UP, pl); printFont(9, 6, ":", fontc[rotspl[0]] * (statusc[0] == 0));
				printMenuButton(3, 7, INPUT_BUTTON_DOWN, pl); printFont(9, 7, ":", fontc[rotspl[0]] * (statusc[0] == 1));
				printMenuButton(3, 8, INPUT_BUTTON_LEFT, pl); printFont(9, 8, ":", fontc[rotspl[0]] * (statusc[0] == 2));
				printMenuButton(3, 9, INPUT_BUTTON_RIGHT, pl); printFont(9, 9, ":", fontc[rotspl[0]] * (statusc[0] == 3));
				...
				if(statusc[0] < 10) {
					...
					for(int32_t i = 0; i < INPUT_GetMaxKey(); i++) {
						if(i != (int32_t)SDL_GetScancodeFromKey(SDLK_ESCAPE, NULL) && INPUT_IsPushKey(i)) {
							bool unmapped = true;
							for (INPUT_Button button = 0; button < statusc[0]; button++) {
					...
					printPrompt(3 + 3, 17, INPUT_PROMPT_OK, digitc[rotspl[0]] * (n % 2));
					printPrompt(3 + 18, 17, INPUT_PROMPT_RETRY, digitc[rotspl[0]] * (n % 2));
					printPrompt(3 + 32, 17, INPUT_PROMPT_CANCEL, digitc[rotspl[0]] * (n % 2));

					if(IsPushPrompt(INPUT_PROMPT_OK)) {
						PlaySE(WAVE_SE_KETTEI);
						for(int32_t i = 0; i < INPUT_BUTTON_COUNT * 2; i++) keyAssign[i] = ncfg[10 + i];
						statusc[0] = 0;
						statusc[2] = 0;
					}
					if(IsPushPrompt(INPUT_PROMPT_RETRY)) {
						PlaySE(WAVE_SE_MOVE);
						for(int32_t i = 0; i < INPUT_BUTTON_COUNT * 2; i++) ncfg[10 + i] = keyAssign[i];
						statusc[0] = 0;
					}
					if(IsPushPrompt(INPUT_PROMPT_CANCEL)) {
						PlaySE(WAVE_SE_MOVE);
						for(int32_t i = 0; i < INPUT_BUTTON_COUNT * 2; i++) ncfg[10 + i] = keyAssign[i];
						statusc[0] = 0;
						statusc[2] = 0;
			...
			// joystick setting
			if (INPUT_GetNumJoys() <= 0) {
				statusc[0] = 0;
				statusc[2] = 0;
			}
			else {
				bool cancel = false;
				#if defined(INPUT_ENABLE_GAME_CONTROLLER) || defined(INPUT_ENABLE_LINUX_GPIO) || defined(INPUT_ENABLE_KEYBOARD)
				switch (INPUT_GetLastDeviceType()) {
				#ifdef INPUT_ENABLE_GAME_CONTROLLER
				case INPUT_DEVICE_TYPE_XBOX:
				case INPUT_DEVICE_TYPE_PLAYSTATION:
				...
				case INPUT_DEVICE_TYPE_KEYBOARD:
					if (statusc[0] < 10 && INPUT_IsPushKey(keyAssign[INPUT_BUTTON_B])) {
						PlaySE(WAVE_SE_MOVE);
						statusc[0] = 0;
						statusc[2] = 0;
						cancel = true;
				...
				if (!cancel) {
					...
					for(int32_t i = 0; i < statusc[0]; i++) {
						switch((INPUT_JoyKeyType)ncfg[ncfgi+5+i*8]) {
						case INPUT_JOY_KEY_AXIS:
					...
					if(statusc[0] < 10) {
						printFont(10, 6 + statusc[0], "_", digitc[rotspl[0]] * (n % 2));
						INPUT_JoyKey pushKey;
						int32_t pushPlayer = -1;
						do {
							if (INPUT_GetPlayerSlotType(pl) != INPUT_PLAYER_SLOT_JOY) {
								break;
							}

							pushKey.player = pl;
							pushKey.guid = INPUT_GetJoyGUID(pl);

							pushKey.type = INPUT_JOY_KEY_AXIS;
							for (int32_t j = 0; j < INPUT_GetMaxJoyAxis(pl); j++)
							{
								pushKey.setting.index = j;

								pushKey.setting.value = -INPUT_DEADZONE_MAX;
								if (INPUT_IsPushJoyKey(&pushKey))
								{
									pushPlayer = pl;
									break;
								}
							...
							pushKey.type = INPUT_JOY_KEY_HAT;
							for (int32_t j = 0; j < INPUT_GetMaxJoyHat(pl); j++)
							{
								pushKey.setting.index = j;
							...
							pushKey.type = INPUT_JOY_KEY_BUTTON;
							for (int32_t j = 0; j < INPUT_GetMaxJoyButton(pl); j++)
							{
						...
						if(IsPushPrompt(INPUT_PROMPT_OK)) {
							PlaySE(WAVE_SE_KETTEI);
							for (INPUT_Button button = 0; button < INPUT_BUTTON_COUNT; button++) {
								INPUT_JoyKey *pljoy = &joyKeyAssign[pl * 10 + button];
								int32_t *plcfg = &ncfg[80 + pl * 10 * 8 + button * 8];
								pljoy->player = plcfg[0];
								for (int32_t i = 0; i < 4; i++) {
									pljoy->guid.data[i] = plcfg[1 + i];
								}
								pljoy->type = (INPUT_JoyKeyType)plcfg[5];
								switch (pljoy->type) {
								case INPUT_JOY_KEY_AXIS:
								case INPUT_JOY_KEY_HAT:
						...
						else if(IsPushPrompt(INPUT_PROMPT_RETRY)) {
							PlaySE(WAVE_SE_MOVE);
							for (INPUT_Button button = 0; button < INPUT_BUTTON_COUNT; button++) {
								int32_t *plcfg = &ncfg[80 + pl * 10 * 8 + button * 8];
								INPUT_JoyKey *pljoy = &joyKeyAssign[pl * 10 + button];
						...
						else if(IsPushPrompt(INPUT_PROMPT_CANCEL)) {
							PlaySE(WAVE_SE_MOVE);
							for (INPUT_Button button = 0; button < 10; button++) {
								int32_t *plcfg = &ncfg[80 + pl * 10 * 8 + button * 8];
								INPUT_JoyKey *pljoy = &joyKeyAssign[pl * 10 + button];
								plcfg[0] = pljoy->player;
								for (int32_t i = 0; i < 4; i++) {
			...
			if (INPUT_GetNumCons() <= 0) {
				statusc[0] = 0;
				statusc[2] = 0;
			}
			else {
				INPUT_ConKey cancelKey = {
					.type = INPUT_CON_KEY_BUTTON,
					.index = 4
				};
				bool cancel = false;
				switch (INPUT_GetLastDeviceType()) {
				...
				case INPUT_DEVICE_TYPE_PLAYSTATION:
				case INPUT_DEVICE_TYPE_NINTENDO:
					if (INPUT_IsPushConKey(-1, &cancelKey)) {
						PlaySE(WAVE_SE_MOVE);
						statusc[0] = 0;
						statusc[2] = 0;
						cancel = true;
				...
				#ifdef INPUT_ENABLE_JOYSTICK
				case INPUT_DEVICE_TYPE_JOYSTICK:
					if (statusc[0] < 8 && INPUT_IsPushJoyKey(&joyKeyAssign[INPUT_BUTTON_B])) {
						PlaySE(WAVE_SE_MOVE);
						statusc[0] = 0;
						statusc[2] = 0;
						cancel = true;
				...
				#ifdef INPUT_ENABLE_LINUX_GPIO
				case INPUT_DEVICE_TYPE_LINUX_GPIO:
					if (statusc[0] < 8 && INPUT_IsPushGPIO(INPUT_BUTTON_B)) {
						PlaySE(WAVE_SE_MOVE);
						statusc[0] = 0;
						statusc[2] = 0;
						cancel = true;
				...
				#ifdef INPUT_ENABLE_KEYBOARD
				case INPUT_DEVICE_TYPE_KEYBOARD:
					if (statusc[0] < 8 && INPUT_IsPushKey(keyAssign[INPUT_BUTTON_B])) {
						PlaySE(WAVE_SE_MOVE);
						statusc[0] = 0;
						statusc[2] = 0;
						cancel = true;
				...
				if (!cancel) {
					...
					printMenuButton(3, 6, INPUT_BUTTON_UP, pl); printFont(4, 6, ":", fontc[rotspl[0]] * (statusc[0] == 0));
					printMenuButton(3, 7, INPUT_BUTTON_DOWN, pl); printFont(4, 7, ":", fontc[rotspl[0]] * (statusc[0] == 1));
					printMenuButton(3, 8, INPUT_BUTTON_LEFT, pl); printFont(4, 8, ":", fontc[rotspl[0]] * (statusc[0] == 2));
					printMenuButton(3, 9, INPUT_BUTTON_RIGHT, pl); printFont(4, 9, ":", fontc[rotspl[0]] * (statusc[0] == 3));
					printFont(3, 10, "A:", fontc[rotspl[0]] * (statusc[0] == 4));
					printFont(3, 11, "B:", fontc[rotspl[0]] * (statusc[0] == 5));
					printFont(3, 12, "C:", fontc[rotspl[0]] * (statusc[0] == 6));
					printFont(3, 13, "D:", fontc[rotspl[0]] * (statusc[0] == 7));
					...
					INPUT_ConKey anyKey;
					anyKey.type = INPUT_CON_KEY_ANY;
					if (conPlayer == -1 && !INPUT_IsPressConKey(-1, &anyKey)) {
						INPUT_ResetLastActiveCon();
						conPlayer = -2;
					}
					else if (conPlayer == -2 && INPUT_GetLastActiveCon() == pl) {
						conPlayer = pl;
					...
					if(statusc[0] < INPUT_BUTTON_GAME_COUNT) {
						...
						bool pushed = false;
						INPUT_ConKey pushKey;

						pushKey.type = INPUT_CON_KEY_BUTTON;
						for (pushKey.index = 0; pushKey.index < INPUT_CON_BUTTON_MAX; pushKey.index++) {
							if (INPUT_IsPushConKey(conPlayer, &pushKey)) {
								pushed = true;
								break;
							}
						}
						...
						if (!pushed) {
							pushKey.type = INPUT_CON_KEY_AXIS;
							for (pushKey.index = 0; pushKey.index < INPUT_CON_AXIS_MAX; pushKey.index++) {
								if (INPUT_IsPushConKey(conPlayer, &pushKey)) {
									pushed = true;
									break;
								}
						...
						if(IsPushPrompt(INPUT_PROMPT_OK)) {
							...
							for(int32_t i = 0; i < 8; i++) {
								INPUT_ConKey* key = &conKeyAssign[pl * 8 + i];
			...
			printMenuButton(7, 28, INPUT_BUTTON_RIGHT, 0);
			printMenuButton(9, 28, INPUT_BUTTON_A, 0);
			...
			for(int32_t pl=0; pl<2; pl++) {
				...
				for(int32_t i=0; i<=7; i++) {
					if(i == 0)      { printMenuButton(3, 7 + i + pl * 10, INPUT_BUTTON_UP, pl); printFont(4, 7 + i + pl * 10, ":", 0); }
					else if(i == 1) { printMenuButton(3, 7 + i + pl * 10, INPUT_BUTTON_DOWN, pl); printFont(4, 7 + i + pl * 10, ":", 0); }
					else if(i == 2) { printMenuButton(3, 7 + i + pl * 10, INPUT_BUTTON_LEFT, pl); printFont(4, 7 + i + pl * 10, ":", 0); }
					else if(i == 3) { printMenuButton(3, 7 + i + pl * 10, INPUT_BUTTON_RIGHT, pl); printFont(4, 7 + i + pl * 10, ":", 0); }
					...
					int32_t j =
						0
						#ifdef INPUT_ENABLE_KEYBOARD
						|| INPUT_IsPressKey(keyAssign[i + 10 * pl])
						#endif
						#ifdef INPUT_ENABLE_JOYSTICK
						|| INPUT_IsPressJoyKey(&joyKeyAssign[i + 10 * pl])	// キー入力状態取得
						#endif
						#ifdef INPUT_ENABLE_GAME_CONTROLLER
						|| INPUT_IsPressConKey(pl, &conKeyAssign[i + 8 * pl])
						#endif
						;
			...
			if( getPressState(0, INPUT_BUTTON_RIGHT) && getPressState(0, INPUT_BUTTON_A) ) {
				PlaySE(WAVE_SE_MOVE);
				statusc[0] = 0;
				statusc[2] = 0;
			}
			...
			for(int32_t pl=0; pl<2; pl++) {
				...
				for(int32_t i=0; i<=7; i++) {
					...
					#ifdef INPUT_ENABLE_JOYSTICK
					INPUT_JoyKey* const key = &joyKeyAssign[i + pl * 10];
					INPUT_JoyGUID getGUID = INPUT_GetJoyGUID(key->player);
					INPUT_JoyGUID zeroGUID = { 0 };
					if (
						INPUT_GetPlayerSlotType(key->player) == INPUT_PLAYER_SLOT_JOY &&
					...
					#ifdef INPUT_ENABLE_GAME_CONTROLLER
					INPUT_ConKey conKey = conKeyAssign[i + pl * 8];
		...
		// video setting
		bool showScreenModeSetting = (ncfg[0] & VIDEO_SCREEN_MODE_WINDOW_TYPE) == VIDEO_SCREEN_MODE_FULLSCREEN || (ncfg[0] & VIDEO_SCREEN_MODE_WINDOW_TYPE) == VIDEO_SCREEN_MODE_WINDOW;
...
#ifdef VIDEO_ENABLE_ALL_SETTINGS
		/* 画面モード */
		switch(ncfg[0] & VIDEO_SCREEN_MODE_WINDOW_TYPE) {
		case VIDEO_SCREEN_MODE_WINDOW: SDL_snprintf(string[0], STRING_LENGTH, "WINDOW"); break;
		case VIDEO_SCREEN_MODE_WINDOW_MAXIMIZED: SDL_snprintf(string[0], STRING_LENGTH, "WINDOW MAXIMIZED"); break;
		case VIDEO_SCREEN_MODE_FULLSCREEN_DESKTOP: SDL_snprintf(string[0], STRING_LENGTH, "FULL SCREEN DESKTOP"); break;
		case VIDEO_SCREEN_MODE_FULLSCREEN: SDL_snprintf(string[0], STRING_LENGTH, "FULL SCREEN"); break;
		...
		SDL_snprintf(string[0], STRING_LENGTH, "%" PRId32, VIDEO_SCREEN_INDEX_DISPLAY_TO_VALUE(ncfg[1]));
		printFont(15, 5 + MENU_AV_SCREEN_INDEX, string[0], (statusc[0] == MENU_AV_SCREEN_INDEX) * (n % 2) * digitc[rotspl[0]]);
		if(ncfg[0] & VIDEO_SCREEN_MODE_VSYNC) SDL_snprintf(string[0], STRING_LENGTH, "ON");
		else SDL_snprintf(string[0], STRING_LENGTH, "OFF");
		printFont(15, 5 + MENU_AV_VSYNC, string[0], (statusc[0] == MENU_AV_VSYNC) * (n % 2) * digitc[rotspl[0]]);
#endif
		SDL_snprintf(string[0], STRING_LENGTH, "%s", ncfg[0] & VIDEO_SCREEN_MODE_DETAIL_LEVEL ? "HIGH (640X480)" : "LOW (320X240)");
		printFont(15, 5 + MENU_AV_DETAIL_LEVEL, string[0], (statusc[0] == MENU_AV_DETAIL_LEVEL) * (n % 2) * digitc[rotspl[0]]);

		if (ncfg[0] & VIDEO_SCREEN_MODE_SCALE_MODE) {
...
#ifdef VIDEO_ENABLE_ALL_SETTINGS
		if (ncfg[0] & VIDEO_SCREEN_MODE_RENDER_LEVEL) {
			SDL_snprintf(string[0], STRING_LENGTH, "HIGH");
		}
		else {
			SDL_snprintf(string[0], STRING_LENGTH, "LOW (%s)", ncfg[0] & VIDEO_SCREEN_MODE_DETAIL_LEVEL ? "640X480" : "320X240");
		}
		printFont(15, 5 + MENU_AV_RENDER_LEVEL, string[0], (statusc[0] == MENU_AV_RENDER_LEVEL) * (n % 2) * digitc[rotspl[0]]);

		/* 画面モード */
		switch(ncfg[0] & VIDEO_SCREEN_MODE_WINDOW_TYPE) {
		case VIDEO_SCREEN_MODE_WINDOW:
			{
			        SDL_snprintf(string[0], STRING_LENGTH, "%" PRId32 "X%" PRId32,
					(!!(ncfg[0] & VIDEO_SCREEN_MODE_DETAIL_LEVEL) + 1) * VIDEO_SCREEN_WIDTH * (VIDEO_SCREEN_INDEX_MODE_TO_VALUE(ncfg[1]) + 1),
					(!!(ncfg[0] & VIDEO_SCREEN_MODE_DETAIL_LEVEL) + 1) * VIDEO_SCREEN_HEIGHT * (VIDEO_SCREEN_INDEX_MODE_TO_VALUE(ncfg[1]) + 1)
				);
				printFont(15, 5 + MENU_AV_SCREEN_MODE, string[0], (statusc[0] == MENU_AV_SCREEN_MODE) * (n % 2) * digitc[rotspl[0]]);
			}
			break;
		case VIDEO_SCREEN_MODE_FULLSCREEN:
			{
				SDL_DisplayMode displayMode;
				VIDEO_GetDisplayMode(VIDEO_SCREEN_INDEX_DISPLAY_TO_VALUE(ncfg[1]), VIDEO_SCREEN_INDEX_MODE_TO_VALUE(ncfg[1]), &displayMode);
				int bpp;
		...
		for(int32_t pl = 0; pl < 2; pl++) {
			if(getPushState(pl, INPUT_BUTTON_A) || getPushState(pl, INPUT_BUTTON_B)) {
				PlaySE(WAVE_SE_KACHI);
				status[0] = 0;
				statusc[0] = 0;
				statusc[1] = 1;
			} else if(padRepeat2(pl), ((mpc2[0] == 1) || ((mpc2[0] > tame3) && (mpc2[0] % tame4 == 0))) && (m = getPressState(pl, INPUT_BUTTON_DOWN) - getPressState(pl, INPUT_BUTTON_UP))) {
				PlaySE(WAVE_SE_MOVE);
				...
				if(m && ((statusc[0] >= MENU_AV_CHANGE_MENU && statusc[0] <= MENU_AV_PLAY_SE) || statusc[0] == MENU_AV_PLAY_BGM || statusc[0] == MENU_AV_BGM_TYPE)) {
					if(statusc[0] == MENU_AV_CHANGE_MENU) {
						...
						ncfg[1] &= ~VIDEO_SCREEN_INDEX_MODE;
						ncfg[0] = (ncfg[0] & ~VIDEO_SCREEN_MODE_WINDOW_TYPE) | ((((ncfg[0] & VIDEO_SCREEN_MODE_WINDOW_TYPE) + VIDEO_SCREEN_MODE_WINDOW_TYPES_COUNT + m)) % VIDEO_SCREEN_MODE_WINDOW_TYPES_COUNT);
						need_reset = 1;
					}
					else if(statusc[0] == MENU_AV_SCREEN_INDEX) {
						ncfg[1] &= ~VIDEO_SCREEN_INDEX_MODE;
						ncfg[1] = (ncfg[1] & ~VIDEO_SCREEN_INDEX_DISPLAY) | VIDEO_SCREEN_INDEX_DISPLAY_TO_SETTING((VIDEO_SCREEN_INDEX_DISPLAY_TO_VALUE(ncfg[1]) + VIDEO_GetMaxDisplayIndex() + m) % VIDEO_GetMaxDisplayIndex());	// displayIndex
						need_reset = 1;
					}
#endif
					else if(statusc[0] == MENU_AV_DETAIL_LEVEL) {
						if((ncfg[0] & VIDEO_SCREEN_MODE_WINDOW_TYPE) == VIDEO_SCREEN_MODE_WINDOW) ncfg[1] &= ~VIDEO_SCREEN_INDEX_MODE;
						ncfg[0] ^= VIDEO_SCREEN_MODE_DETAIL_LEVEL;
						load = 1;
						need_reset = 1;
					}
#ifdef VIDEO_ENABLE_ALL_SETTINGS
					else if(statusc[0] == MENU_AV_VSYNC) {
						ncfg[0] ^= VIDEO_SCREEN_MODE_VSYNC;
						need_reset = 1;
					}
#endif
					else if(statusc[0] == MENU_AV_SCALE_MODE) {
						ncfg[0] ^= VIDEO_SCREEN_MODE_SCALE_MODE;
						need_reset = 1;
					}
#ifdef VIDEO_ENABLE_ALL_SETTINGS
					else if(statusc[0] == MENU_AV_RENDER_LEVEL) {
						ncfg[0] ^= VIDEO_SCREEN_MODE_RENDER_LEVEL;
						need_reset = 1;
					}
					else if(statusc[0] == MENU_AV_SCREEN_MODE) {
						switch(ncfg[0] & VIDEO_SCREEN_MODE_WINDOW_TYPE) {
						case VIDEO_SCREEN_MODE_WINDOW: {
							const SDL_DisplayMode* displayMode = VIDEO_GetDesktopDisplayMode(VIDEO_SCREEN_INDEX_DISPLAY_TO_VALUE(ncfg[1]));
							int baseW = (!!(ncfg[0] & VIDEO_SCREEN_MODE_DETAIL_LEVEL) + 1) * VIDEO_SCREEN_WIDTH;
							int baseH = (!!(ncfg[0] & VIDEO_SCREEN_MODE_DETAIL_LEVEL) + 1) * VIDEO_SCREEN_HEIGHT;
							int modeCount;
							...
							int modeIndex = VIDEO_SCREEN_INDEX_MODE_TO_VALUE(ncfg[1]);
							modeIndex = (modeIndex + modeCount + m) % modeCount;
							ncfg[1] = (ncfg[1] & ~VIDEO_SCREEN_INDEX_MODE) | VIDEO_SCREEN_INDEX_MODE_TO_SETTING(modeIndex);
							break;
						}
						case VIDEO_SCREEN_MODE_FULLSCREEN: {
							int modeIndex = VIDEO_SCREEN_INDEX_MODE_TO_VALUE(ncfg[1]);
							int maxDisplayMode = VIDEO_GetMaxDisplayMode(VIDEO_SCREEN_INDEX_DISPLAY_TO_VALUE(ncfg[1]));
					...
					if(statusc[0] == MENU_AV_SE_VOLUME) {
						...
						AUDIO_SetMusicVolume(bgmvolume_temp);
					}
				}
			}
		}
