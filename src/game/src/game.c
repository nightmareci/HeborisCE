...
bool inmenu = true;
INPUT_DeviceType lastInputType = INPUT_DEVICE_TYPE_NULL;
INPUT_DeviceType lastPlayerInputType[2] = { INPUT_DEVICE_TYPE_NULL, INPUT_DEVICE_TYPE_NULL };

bool	lastPushUp[2] = { 0 };
bool	lastPushDown[2] = { 0 };
bool	lastPushLeft[2] = { 0 };
bool	lastPushRight[2] = { 0 };
INPUT_Button	pushDirection[2] = { INPUT_BUTTON_NULL, INPUT_BUTTON_NULL };

bool	lastPressUp[2] = { 0 };
bool	lastPressDown[2] = { 0 };
bool	lastPressLeft[2] = { 0 };
bool	lastPressRight[2] = { 0 };
INPUT_Button	pressDirection[2] = { INPUT_BUTTON_NULL, INPUT_BUTTON_NULL };
...
void gameUpdate() {
	...
	case MAIN_START: {
		...
		VIDEO_ScreenModeFlag oldScreenMode = screenMode;
		int32_t oldScreenIndex = screenIndex;
		VIDEO_SetScreen(&screenMode, &screenIndex);
		if ( screenMode != oldScreenMode || screenIndex != oldScreenIndex )
		{
			SaveConfig();
		}

		MAIN_SetRenderWhileSkippingFrames(true);
		MAIN_ResetFrameStep();
		break;
	}
	...
	case MAIN_INIT_TEXT:
		...
		{
			VIDEO_SetTextLayerSize(layer, 12);
		}

		VIDEO_EnableTextLayer(1, 10, 220);
		VIDEO_PutTextLayerString(1, version);
		for ( int layer = 1 ; layer <= 5 ; layer ++ )
		{
	...
	case MAIN_INIT_LOAD_1: {
		...
		// グラフィック読み込み
		VIDEO_EnableTextLayer(4, 10, 23);
		VIDEO_PutTextLayerString(4, "Graphics Loading");
		for ( int layer = 1 ; layer <= 5 ; layer ++ )
		{
			VIDEO_DrawTextLayer(layer);
		}
		ExBltRect(PLANE_LOADING, 160, 0, 160 * x, 240 * y, 160, 240);
		break;
	}
	...
	case MAIN_INIT_LOAD_2:
		...
		// 効果音読み込み
		if(se) {
			VIDEO_EnableTextLayer(1, 10, 36);
			VIDEO_PutTextLayerString(1, "Sound Effect Loading");
			for ( int layer = 1 ; layer <= 5 ; layer ++ )
			{
				VIDEO_DrawTextLayer(layer);
			}
	...
	case MAIN_INIT_LOAD_3:
		...
		if(bgm) {
			...
			VIDEO_EnableTextLayer(5, 10, 49);
			VIDEO_PutTextLayerString(5, "BGM Loading");
			for ( int i = 1 ; i <= 5 ; i ++ )
			{
				VIDEO_DrawTextLayer(i);
			}
			ExBltRect(PLANE_LOADING, 160, 0, 160 * x, 240 * y, 160, 240);
		}
		else {
	...
	case MAIN_INIT_END: {
		...
		MAIN_SetRenderWhileSkippingFrames(false);
		...
		if(MAIN_GetFPS() != max_fps_2) {
			MAIN_SetFPS(max_fps_2);
		}
		else {
			MAIN_ResetFrameStep();
		}

		MAIN_WaitToFinishLoading();
	...
	case MAIN_RESET_KEYBOARD: {
		...
		if (
			!enterResetKeys &&
			(
				(lastEnterPressed && !INPUT_IsPressKey(SDL_GetScancodeFromKey(SDLK_RETURN, NULL))) ||
				(lastEscapePressed && !INPUT_IsPressKey(SDL_GetScancodeFromKey(SDLK_ESCAPE, NULL)))
			)
		) {
...
void gameExecute() {
	...
	if(playback && !demo) {
		playback = 0;
		if(getPushState(0, INPUT_BUTTON_A) || getPushState(1, INPUT_BUTTON_A)) fast = 5;
		if(!(getPressState(0, INPUT_BUTTON_A) || getPressState(1, INPUT_BUTTON_A))) fast = 0;
		if(!fast && (getPressState(0, INPUT_BUTTON_UP) || getPressState(1, INPUT_BUTTON_UP))) fast = 3;
		if(!fast && (getPressState(0, INPUT_BUTTON_DOWN) || getPressState(1, INPUT_BUTTON_DOWN))) fast = 1;
		if(getPushState(0, INPUT_BUTTON_B) || getPushState(1, INPUT_BUTTON_B)) {
			if(!tmp_maxPlay)
				maxPlay = 0;
			restoreSetups();
...
bool lastProc(void) {
	...
	if((playback) && (!demo)){
		if(gameMode[0] != 4){
			ExBltRect(PLANE_ROLLMARK,29 + 128 * maxPlay + 213 * ((gameMode[0] == 8) && (!maxPlay)), 65, (time2[0]%6)*48 ,(time2[0]%30/6)*48,48,48);
			ExBltRect(PLANE_HEBOFONT5,34 + 128 * maxPlay + 213 * ((gameMode[0] == 8) && (!maxPlay)), 73, 35, 7*6, 22, 7);//ROTSの文字
			ExBltRect(PLANE_ROT,22 + 128 * maxPlay + 213 * ((gameMode[0] == 8) && (!maxPlay)), 84 - (3 * (getDrawRate() == 1)), 64*rotspl[0] ,32*fontc[rotspl[0]] + (7 * (getDrawRate() == 1)),64,12 + (5 * (getDrawRate() == 1)));
			if(fast != 0){
	...
	if(demo) {
		if(maxPlay) {
			...
			ExBltRect(PLANE_ROT,31 , 35, 64*rotspl[0] ,32*fontc[rotspl[0]],64,32);

			if(!english)
				ExBltRect(PLANE_ROT,31, 69, 64*rotspl[0] ,320,64,32);
			else
				ExBltRect(PLANE_ROT,31, 69, 64*rotspl[0] ,352,64,32);
			ExBltRect(PLANE_ROT,31, 102 ,64*rotspl[0],416,64,32);
			showControl();
		}
		ExBltRect(PLANE_ROLLMARK,136, 140, (count%6)*48 ,(count%30/6)*48,48,48);

		ExBltRect(PLANE_LINE, 0, 176,  count % 320, 20, 320 - (count % 320), 8);
		ExBltRect(PLANE_LINE, 320 - (count % 320), 176,  0, 20, count % 320, 8);

		ExBltRect(PLANE_LINE, count % 320, 144,  0, 28, 320 - (count % 320), 8);
		ExBltRect(PLANE_LINE, 0, 144, 320 - (count % 320), 28, count % 320, 8);

		if(count % 80 < 40) {
			ExBltRect(PLANE_TEXT2,53,154,0,204,213,19);
		}

		if(getDrawRate() != 1) ExBltRect(PLANE_HEBOSPR,101,120 - 23 * maxPlay,99,83,118,29);

		demo = 0;
		cpu_flag[0] = 0;
		cpu_flag[1] = 0;

		if(getPushState(0, INPUT_BUTTON_A) || getPushState(1, INPUT_BUTTON_A)) {
			StopAllBGM();
	...
	if(!playback) {
		if(getPressState(0, INPUT_BUTTON_PAUSE)) mpc4[0]++;
		else mpc4[0] = 0;
	} else {
		mpc4[0] = 0;
	}

	if(getPushState(0, INPUT_BUTTON_PAUSE) || mpc4[0] == 1) {
		if( ((status[0] >= 3) && (status[0] <= 8) && (status[0] != 7)) || (status[0] == 13) || (status[0] == 15) ||
			...
			if(pauseGame[0]) {
				...
				AUDIO_ResumeWave(AUDIO_WAVE_NUM_ALL);
			} else {
				// ポーズ
				setGamePause(0, true);
				AUDIO_PauseWave(AUDIO_WAVE_NUM_ALL);
			}
	...
	if(!playback) {
		if(getPressState(1, INPUT_BUTTON_PAUSE)) mpc4[1]++;
		else mpc4[1] = 0;
	} else {
		mpc4[0] = 0;
	}

	if(getPushState(1, INPUT_BUTTON_PAUSE) || mpc4[1] == 1) {
			...
			for(tmp = 0; tmp <= 1 ; tmp++)
				for(i = 0; i < 20; i++)
					for(j = 0 + 4 * pl; j < 10 + 4 * pl; j++)
						ExBltRect(PLANE_TOMOYO_EH_FADE, (24+(192*tmp))+(j * 8), (i+1+ 4) * 8, tmp*8, fadec*8, 8, 8);
		}
	...
	if(thunder_timer){
		...
		VIDEO_SetPlaneDrawOffset(i-(i/2),j-(j/2));
		thunder_timer--;
	}else{
	...
	if(!hide_fps) {
		SDL_snprintf(string[0], STRING_LENGTH, "%3d/%2dFPS", MAIN_GetRealFPS(), MAIN_GetFPS() );	// FPSの取得(測定値)
		printTinyFont(130, 233, string[0]);
	}

	#ifdef INPUT_ENABLE_KEYBOARD
	// NEXT隠し
	if(INPUT_IsPushKey(dispnextkey[0]) && (!demo) && (!playback) && (!death_plus[0]) && (!hebo_plus[0])&&(!heboGB[0])&&(!onRecord[0])) {
		// next表示個数は0〜6 #1.60c7q3
		hnext[0]++;
		if(hnext[0] > 6) hnext[0] = 0;
		if((hnext[0] > max_hnext[0]) && onRecord[0]) max_hnext[0] = hnext[0];
	}
	if(INPUT_IsPushKey(dispnextkey[1]) && (!demo) && (!playback) && (!death_plus[1]) && (!hebo_plus[1])&&(!heboGB[1])&&(!onRecord[1])) {
		hnext[1]++;
...
void title(void) {
	...
	if (init) {
		...
		if (bgm) {
			if(wavebgm & WAVE_BGM_SIMPLE) {	// No.30→38に変更 #1.60c7i2
				if(!AUDIO_IsMusicPlaying()) {
					AUDIO_PlayMusic();
					AUDIO_SetMusicVolume(bgmvolume);
				}
			} else {
				if(!AUDIO_IsWavePlaying(WAVE_BGM_TITLE)) AUDIO_PlayWave(WAVE_BGM_TITLE);
			}
		}
		else {
			if (AUDIO_IsMusicPlaying()) AUDIO_StopMusic();
	...
	// 背景を描く
	if (!title_mov_f) {
		ExBlt(PLANE_TITLE, 0, 0);
	}
	else {
		ExBltRect(PLANE_TITLE,0,0,(title_acnt / 10) * 320,(title_acnt % 10) * 240, 320, 240);
		if(count % mov_inter == 0) title_acnt++;
		if(title_acnt > title_mov_f -1) title_acnt = 0;
	}
	ExBltRect(PLANE_LINE, 0, 106,  count % 320, 4, 320 - (count % 320), 8);
	ExBltRect(PLANE_LINE, 320 - (count % 320), 106,  0, 4, count % 320, 8);

	ExBltRect(PLANE_LINE, count % 320, 20,  0, 4, 320 - (count % 320), 8);
	ExBltRect(PLANE_LINE, 0, 20, 320 - (count % 320), 4, count % 320, 8);


	ExBltRect(PLANE_LINE, 0, 176 + ofs,  count % 320, 20, 320 - (count % 320), 8);
	ExBltRect(PLANE_LINE, 320 - (count % 320), 176 + ofs,  0, 20, count % 320, 8);

	ExBltRect(PLANE_LINE, count % 320, 144 - (ofs / 5),  0, 28, 320 - (count % 320), 8);
	ExBltRect(PLANE_LINE, 0, 144 - (ofs / 5), 320 - (count % 320), 28, count % 320, 8);

	ExBlt(PLANE_LOGO, 0, 0);
...
void versusInit(int32_t player) {
	...
	if( (!getPushState(player, INPUT_BUTTON_B) && (gameMode[player] == 6) && (!randommode[player])) || (nextblock ==11)|| ((p_nextblock ==11)&&(gameMode[player] == 5))) {
...
//	hole[player] = SDL_rand(10);	//擬似乱数のためにブロックシャッターへ移動
...
//	代わりに「i」を用いてください。
	...
	if( ((pinch[0])&&(onRecord[0])) || ((pinch[1])&&(onRecord[1])) ) {
		if( !AUDIO_IsWavePlaying(WAVE_SE_PINCH) ) PlaySE(WAVE_SE_PINCH);
	} else {
	...
	for(i = 0; i < 1 + maxPlay; i++) {
		...
		if(pauseGame[i]) {
			...
			if(gameMode[i] == 5){
				...
				if((item_mode[i])||(novice_mode[i])){
					...
					if(getPressState(i, INPUT_BUTTON_C)){
						if(getPushState(i, INPUT_BUTTON_LEFT)){
							PlaySE(WAVE_SE_MOVE);
							item_nblk[0 + i * 6]--;
							if(item_nblk[0 + i * 6] < 0) item_nblk[0 + i * 6] = item_num;
							item_name[i] = item_nblk[0 + i * 6];
						}
						if(getPushState(i, INPUT_BUTTON_RIGHT)){
							PlaySE(WAVE_SE_MOVE);
							item_nblk[0 + i * 6]++;
		...
		if( (gameMode[i] == 4) || (item_mode[i]) ) {
			if( (isrotatelock[i]) && (item_timer[i] <= 0) ) {
				...
				// フィーバー
				if(!isfever[1-i])
					AUDIO_StopWave(WAVE_BGM_FEVER);
				if((!AUDIO_IsWavePlaying(WAVE_BGM_START+bgmlv)) && (!isfever[1-i]) && (!ending[i]) && (timeOn[i]))
					AUDIO_PlayWave(WAVE_BGM_START+bgmlv);
				isfever[i] = 0;
				item_timer[i] = 0;
...
int32_t doGiveup() {
	...
	if(getPressState(0, INPUT_BUTTON_GIVE_UP)) mpc3[0]++;
	else mpc3[0] = 0;

	if(getPressState(1, INPUT_BUTTON_GIVE_UP)) mpc3[1]++;
	else mpc3[1] = 0;
...
void increment_time(int32_t player) {
	...
	if(onRecord[player] != 0) {
		if( (!playback) && (!demo) && (replay_save[player]) && (time2[player] < REPLAY_TIME_MAX) && (!fpbas_mode[player])) {
			...
			temp = 0;
			for(j = 0; j < INPUT_BUTTON_GAME_COUNT; j++)
				temp = temp | (getPressState(player, j) << j) | (getPushState(player, j) << (j + INPUT_BUTTON_GAME_COUNT));
	...
	if( (timeOn[player]) && (gameMode[player] == 4) && (vs_time != 0) ) {
		...
		// 10秒前の警告音 #1.60c7r9
		if(ltime[player] == 10 * 60) PlaySE(WAVE_SE_HURRYUP);
		...
		if(ltime[player] <= 0) {
			...
			ltime[0] = 0;
			ltime[1] = 0;
			PlaySE(WAVE_SE_TIMEOVER);//タイムオーバー音
			if(wintype == 1){
				if(li[0] == li[1]) {
					// レベルが同じなら引き分け
					status[0] = 7;
	...
	if( (timeOn[player]) && (gameMode[player] == 6) ) {
		...
		else if(stage[player] <= 19)
		{
			if(stime[player] > 0) {
				...
				if(stime[player] == 10 * 60) PlaySE(WAVE_SE_HURRYUP);
				if(stime[player] == 0) PlaySE(WAVE_SE_TIMEOVER);
			}
		}
	}
...
void statJoinwait(int32_t player) {
	...
	if(count % 80 > 10) {
		if(!playback) {
			ExBltRect(PLANE_TEXT2, 110+192 * player -96 * maxPlay , 85 , 0 + 100 * player, 50, 100, 65);
		}
	}

	// ボタンが利きにくいのを修正#1.60cf
	if((getPushState(player, INPUT_BUTTON_A)) && (tomoyo_domirror[1-player] == 0) && !((player == 1) && ((domirror) && (mirror)) ) && (!bgfadesw)) {
		StopSE(WAVE_SE_GAMEOVER);
		PlaySE(WAVE_SE_KETTEI);

		if(!(wavebgm & WAVE_BGM_SIMPLE)) {
			if(!AUDIO_IsWavePlaying(WAVE_BGM_SELECT)) AUDIO_PlayWave(WAVE_BGM_SELECT);
		}
...
void statSelectMode(int32_t player) {
	...
	if (bgm) {
		if( (!AUDIO_IsMusicPlaying()) && (wavebgm & WAVE_BGM_SIMPLE) ) {
			AUDIO_PlayMusic();
			AUDIO_SetMusicVolume(bgmvolume);
		} else if(((status[1 - player] == 0) || (status[1 - player] == 10)) && (!AUDIO_IsWavePlaying(WAVE_BGM_SELECT)) && !(wavebgm & WAVE_BGM_SIMPLE) ) {
	...
	if(statusc[player * 10 + 2] == 0) {
		...
		ExBltRect(PLANE_LINE, 120 + 192 * player -96 * maxPlay , 66,  (statusc[player * 10 + 1] % 40) * 3, 28, 30, 8);
		ExBltRect(PLANE_LINE, 120 + 192 * player -96 * maxPlay , 75,  160 - ((statusc[player * 10 + 1] % 40) * 3), 20, 30, 8);

		ExBltRect(PLANE_LINE, 170 + 192 * player -96 * maxPlay , 66,  50 + ((statusc[player * 10 + 1] % 40) * 3), 28, 30, 8);
		ExBltRect(PLANE_LINE, 170 + 192 * player -96 * maxPlay , 75,  210 - ((statusc[player * 10 + 1] % 40) * 3), 20, 30, 8);

		if(statusc[player * 10 + 4] < 0){
			ExBltRect(PLANE_ROT,128 + ((16+statusc[player * 10 + 4]) * 4) + 192 * player -96 * maxPlay , 84,
				(64*(rotspl[player]+1))*(rotspl[player]<8),32*(fontc[(rotspl[player]+1)*(rotspl[player]<8)]),
				(statusc[player * 10 + 4]* -4),32);
			ExBltRect(PLANE_ROT,128 + 192 * player -96 * maxPlay , 84,
				(64*rotspl[player])+(statusc[player * 10 + 4] * -4),32*fontc[rotspl[player]],((16+statusc[player * 10 + 4])* 4),32);
		} else if(statusc[player * 10 + 4] > 0){
			ExBltRect(PLANE_ROT,128 + (statusc[player * 10 + 4] * 4) + 192 * player -96 * maxPlay , 84,
		...
		} else ExBltRect(PLANE_ROT,128 + 192 * player -96 * maxPlay , 84, 64*rotspl[player] ,32*fontc[rotspl[player]],64,32);

			ExBltRect(PLANE_ROT,128 + 192 * player -96 * maxPlay , 64, 64*rotspl[player] ,384,64,21);

			ExBltRect(PLANE_ROLLMARK,208 + 48 * player -96 * maxPlay , 40, (statusc[player * 10 + 1]%6)*48 ,(statusc[player * 10 + 1]%30/6)*48,48,48);
		...
		if(getDrawRate() != 1){
			//スクロールする回転の説明
			ExBltRect(PLANE_ROTSTEXT,120 + 192 * player -96 * maxPlay , 118, statusc[player * 10 + 1] % 320 ,(20*rotspl[player]) + (10 * (english)), 80, 10);
		}
		//レバーとボタンの画像
		ExBltRect(PLANE_TEXT,126 + 192 * player -96 * maxPlay , 128, 224 ,0+(40*((isWRule(player)) && (w_reverse)))+(80*(rotspl[player] >= 7)),68,40);
		if(!maxPlay) ExBltRect(PLANE_ROT,211, 98 ,64*rotspl[player],416,64,32);
		if(!english)	//落下特性（日本語）
			ExBltRect(PLANE_ROT,128 + 192 * player -96 * maxPlay , 168, 64*rotspl[player] ,320,64,32);
		else			//落下特性（English）
			ExBltRect(PLANE_ROT,128 + 192 * player -96 * maxPlay , 168, 64*rotspl[player] ,352,64,32);
	} else if(statusc[player * 10 + 2] == 1) {
		// モードセレクト
		printSMALLFont(124 + 192 * player - 96 * maxPlay, 54, "  Game Mode", 4);

		ExBltRect(PLANE_LINE, 120 + 192 * player -96 * maxPlay , 63,  (count % 40) * 3, 28, 80, 8);
		ExBltRect(PLANE_LINE, 120 + 192 * player -96 * maxPlay , 160,  160 - ((count % 40) * 3), 20, 80, 8);
		...
		//biginner
		if(gameMode[player]<9){
			ExBltRect(PLANE_GAMEMODEFONT, 130+192 * player -96 * maxPlay , 71 , 72 * (novice_mode[player]), 0, 72, 9);//BEGINNERの文字
			if(statusc[player * 10 + 2] == 1)	{
				if(novice_mode[player]){
					ExBltRect(PLANE_HEBOFONT5, 144+192 * player -96 * maxPlay, 80, 0, 7*20, 70, 7);
				}else{
					ExBltRect(PLANE_HEBOFONT5, 144+192 * player -96 * maxPlay, 80, 0, 7*21, 70, 7);
				}
			}
		}
		//master
		if(gameMode[player]<10){
			ExBltRect(PLANE_GAMEMODEFONT, 130+192 * player -96 * maxPlay , 87-(gameMode[player]==9)*16 , 72 * enable_grade[player], 9, 72, 9);//MASTER
			if(((gameMode[player] >= 1) && (gameMode[player] <= 3)) && (item_mode[player])){
				if(statusc[player * 10 + 2] == 1)
					ExBltRect(PLANE_HEBOFONT5, (18 + 24 * player - 12 * maxPlay)*8, (10 + (gameMode[player] - ((gameMode[player] >= 6)+(gameMode[player] >= 9)) * 2)*2)*8, 0, 7*22, 50, 7);
				else
					printFont(19 + 24 * player - 12 * maxPlay, 19 , "ITEM", 5);
			}
			if(((gameMode[player] == 1) || (gameMode[player] == 2)) && (hebo_plus[player])){
				if(statusc[player * 10 + 2] == 1)
					ExBltRect(PLANE_HEBOFONT5, (18 + 24 * player - 12 * maxPlay)*8, (10 + (gameMode[player] - ((gameMode[player] >= 6)+(gameMode[player] >= 9)) * 2)*2)*8, 0, 7*23, 50, 7);
				else
					printFont(17 + 24 * player - 12 * maxPlay, 19 , "HEBO+", 9);
			}
		}
		//20G
		ExBltRect(PLANE_GAMEMODEFONT, 130+192 * player -96 * maxPlay , 103-(gameMode[player]>=9)*16 -(gameMode[player]==10)*16, 72 * enable_grade[player], 18, 72, 9);//20G
		//devil
		ExBltRect(PLANE_GAMEMODEFONT, 130+192 * player -96 * maxPlay , 119-(gameMode[player]>=9)*16 -(gameMode[player]==10)*16, 72 * devil_minus[player], 27, 72, 9);
		if((gameMode[player] == 3) && (death_plus[player])){
			if(statusc[player * 10 + 2] == 1)
				ExBltRect(PLANE_HEBOFONT5, (19 + 24 * player - 12 * maxPlay)*8, (10 + (gameMode[player] - (gameMode[player] >= 6) * 2)*2)*8, 0, 7*24, 40, 7);
			else
				printFont(19 + 24 * player - 12 * maxPlay, 19 , "DEATH+", 2);
		}
		//tomoyo
		ExBltRect(PLANE_GAMEMODEFONT, 130+192 * player -96 * maxPlay , 135-(gameMode[player]>=9)*16 -(gameMode[player]==10)*16, 72 * tomoyo_opt[player], 54, 72, 9);
		if((gameMode[player] == 6) && (tomoyo_opt[player] == 4) && (fpbas_mode[player])){
			if(statusc[player * 10 + 2] == 1)
				ExBltRect(PLANE_HEBOFONT5, (21 + 24 * player - 12 * maxPlay)*8, (10 + (gameMode[player] - ((gameMode[player] >= 6)+(gameMode[player] >= 9)) * 2)*2)*8, 70, 7*8, 30, 7);
			else if(statusc[player * 10 + 2] == 2)
				printFont(15 + 24 * player - 12 * maxPlay, 19 , "BASIC", 3);
		}
		if((gameMode[player] == 6) && (tomoyo_opt[player] == 4) &&(!fpbas_mode[player])){
			if(statusc[player * 10 + 2] == 1)
				ExBltRect(PLANE_HEBOFONT5, (21 + 24 * player - 12 * maxPlay)*8, (10 + (gameMode[player] - ((gameMode[player] >= 6)+(gameMode[player] >= 9)) * 2)*2)*8, 70, 7*7, 30, 7);
			else if(statusc[player * 10 + 2] == 2)
				printFont(15 + 24 * player - 12 * maxPlay, 19 , "18MIN", 3);
		}
		//ace
		ExBltRect(PLANE_GAMEMODEFONT, 130+192 * player -96 * maxPlay , 151-(gameMode[player]>=9)*16-(gameMode[player]==10)*16 , 72 * anothermode[player], 63, 72, 9);
		//std
		if(gameMode[player]>=9){
			ExBltRect(PLANE_GAMEMODEFONT, 130+192 * player -96 * maxPlay , 151 -(gameMode[player]==10)*16, 72 * std_opt[player], 81, 72, 9);
			ExBltRect(PLANE_HEBOFONT5, (18 + 24 * player - 12 * maxPlay)*8, (20 -(gameMode[player] >= 10)* 2)*8, 0, 7*(25 + std_opt[player]), 50, 7);
			if(relaymode[player]){
				if(statusc[player * 10 + 2] == 1)
		...
		//ori
		if(gameMode[player]==10){
			ExBltRect(PLANE_GAMEMODEFONT, 130+192 * player -96 * maxPlay , 151 , 72 * ori_opt[player], 90, 72, 9);
			ExBltRect(PLANE_HEBOFONT5, 150+192 * player -96 * maxPlay, 162, 70, 7*(2+ori_opt[player]), 35, 7);
		}
		...
		if(textguide){
			...
			if(gameMode[player] == 6){
				printMenuButton(26 + 10 * player - 12 * maxPlay, 15, INPUT_BUTTON_C, player);
			...
			if(gameMode[player] == 3){
				printMenuButton(26 + 7 * player - 12 * maxPlay, 17, INPUT_BUTTON_D, player);
		...
		if ( getDrawRate() == 1 ){
			if((gameMode[player] >= 6) && (gameMode[player] != 10)) {
				ExBltRect(PLANE_TEXT, 119 + player * 192 - 96 * maxPlay, 168, 0+(80*(english)), (gameMode[player] - 2) * 32, 80, 32);
			} else if(gameMode[player] != 10) {
				ExBltRect(PLANE_TEXT, 119 + player * 192 - 96 * maxPlay, 168, 0+(80*(english)), gameMode[player] * 32, 80, 32);
			}
			...
			if(gameMode[player] == 0){
				ExBltRect(PLANE_TEXT, 120 + player * 192 - 96 * maxPlay, 192,statusc[player * 10 + 1] % 320,264 + (16 * novice_mode[player]) + (8 * english),80,8);
			}else if(gameMode[player] <= 2){
				ExBltRect(PLANE_TEXT, 120 + player * 192 - 96 * maxPlay, 192,statusc[player * 10 + 1] % 320,296 + (80 * (gameMode[player] - 1)) +(16 * enable_grade[player]) + (8 * english),80,8);
			}else if(gameMode[player] == 3){
				ExBltRect(PLANE_TEXT, 120 + player * 192 - 96 * maxPlay, 192,statusc[player * 10 + 1] % 320,456 + (16 * devil_minus[player]) + (8 * english),80,8);
			}else if(gameMode[player] == 7) {
				ExBltRect(PLANE_TEXT, 119 + player * 192 - 96 * maxPlay, 192, 160, ((gameMode[player] - 2)*32)+(8*anothermode[player])+(32*(english)), 80, 8);
			} else if(gameMode[player] == 6) {
				if(tomoyo_opt[player] == 4)
					ExBltRect(PLANE_TEXT, 119 + player * 192 - 96 * maxPlay, 192, 192, 128, 28, 8);//FPの文字
				else
					ExBltRect(PLANE_TEXT, 119 + player * 192 - 96 * maxPlay, 192, 160, 128+(tomoyo_opt[player]*8), 32, 8);
			}
		}
	}
	...
	if( getPressState(player, INPUT_BUTTON_C) ) {
		if(statusc[player * 10 + 2] == 1){
			if(gameMode[player] == 6)//RANDOM
				ExBltRect(PLANE_HEBOFONT5, (16 + 24 * player - 12 * maxPlay)*8, (10 + (gameMode[player] - ((gameMode[player] >= 6)+(gameMode[player] >= 9)) * 2)*2)*8, 70, 7*6, 33, 7);
			else//BIG
				ExBltRect(PLANE_HEBOFONT5, (16 + 24 * player - 12 * maxPlay)*8, (10 + (gameMode[player] - ((gameMode[player] >= 6)+(gameMode[player] >= 9)) * 2)*2)*8, 0, 7*1, 20, 7);
		}
		else if(statusc[player * 10 + 2] == 2){
		if(gameMode[player] == 6)
	...
	// HOLDボタン
	// アイテムモードとか、DEATH+とか
	if(getPushState(player, INPUT_BUTTON_D) && (statusc[player * 10 + 2] == 1)) {
		PlaySE(WAVE_SE_MOVE);
		if((gameMode[player] == 6)&&(tomoyo_opt[player]==4)){
			if(!fpbas_mode[player])
			fpbas_mode[player]=1;
		...
		if(statusc[player * 10 + 0]) {
			...
			if( (!mini_select) && (gameMode[player] == 6) ) {
				...
				if(gameMode[player] == 6) {
					...
					if(((gameMode[player] == 1) || (gameMode[player] == 2))){
						if((((enable_randexam==1) && (!item_mode[player]) && (!IsBig[player]) && (!hebo_plus[player])) && (SDL_rand(10) < 2)) ||(getPressState(player, INPUT_BUTTON_D))){
							item_mode[player] = 0;
							hebo_plus[player] = 0;
							IsBig[player] = 0;
...
//	printFont(19 + 24 * player - 12 * maxPlay, 7, "WAITS", 4);
	...
	if(statusc[player * 10] > 0) {
		printMenuButton(15 + 24 * player - 12 * maxPlay, 22, INPUT_BUTTON_A, player);
...
//	printFont(14 + 24 * player - 12 * maxPlay, 9 + statusc[player * 10], "b", 1);
	...
	if(getPushState(player, INPUT_BUTTON_B)) {		// Bボタンでモード選択画面に戻る
		sp[player] = 1;
		PlaySE(WAVE_SE_MOVE);
		status[player] = 1;				// ブロックシャッター実行
		statusc[player * 10] = 0;			// ステータスカウンタを0に
	...
	if((statusc[player * 10 + 1] > 15 * 60) || (getPushState(player, INPUT_BUTTON_A))) {
		if(statusc[player * 10] == 0) {
			// FAVORITES関連
...
//	int32_t r_start, r_end, g_start, g_end;
	...
	if( (statusc[player * 10] == 0) && (gameMode[player] == 6) ) {
		...
		if(!(wavebgm & WAVE_BGM_SIMPLE)) {
			if( !AUDIO_IsWavePlaying(WAVE_BGM_START+bgmlv) ) AUDIO_PlayWave(WAVE_BGM_START+bgmlv);
		}
	...
	if(ready_go_style == 0){	// HEBORIS
		if(statusc[player * 10] < r_start[player]) {
			...
			ExBltRect(PLANE_HEBOSPR, 121 + player * 192 - 96 * maxPlay, 239 - ((statusc[player * 10] - r_start[player]) * (1500 / (r_end[player] - r_start[player]))) / 100 * 8, 0, 0, 78, 17);
		} else if((statusc[player * 10] < g_start[player]) && (g_start[player] >= r_end[player])) {
			ExBltRect(PLANE_HEBOSPR, 121 + player * 192 - 96 * maxPlay, 119, 0, 0, 78, 17);
		} else if(g_end[player] > g_start[player]) {
			ExBltRect(PLANE_HEBOSPR, 121 + player * 192 - 96 * maxPlay, 119 - ((statusc[player * 10] - g_start[player]) * (1500 / (g_end[player] - g_start[player]))) / 100 * 8, 0, 0, 78, 17);
			ExBltRectScaled(PLANE_HEBOSPR, 117 + player * 192 - ((g_end[player] - statusc[player * 10]) * (1500 / (g_end[player] - g_start[player]))) / 100 * 86 / 16 - 96 * maxPlay,
		...
		if(statusc[player * 10] < r_start[player]) {
			ExBltRect(PLANE_LINE, 120 + 192 * player -96 * maxPlay , 98  - (r_start[player] - statusc[player * 10]),  (statusc[player * 10] % 40) * 3, 28, 80, 8);
			ExBltRect(PLANE_LINE, 120 + 192 * player -96 * maxPlay , 126 + (r_start[player] - statusc[player * 10]),  160 - ((statusc[player * 10] % 40) * 3), 20, 80, 8);
		} else if( (statusc[player * 10] >= r_start[player]) && (statusc[player * 10] < r_end[player]) ) {
			ExBltRect(PLANE_LINE, 120 + 192 * player -96 * maxPlay , 98  ,  (statusc[player * 10] % 40) * 3, 28, 80, 8);
			ExBltRect(PLANE_LINE, 120 + 192 * player -96 * maxPlay , 126 ,  160 - ((statusc[player * 10] % 40) * 3), 20, 80, 8);
			// READY
			ExBltRect(PLANE_TEXT2, 110+192 * player -96 * maxPlay , 91 , 200, 0, 100, 50);
			...
			ExBltRect(PLANE_LINE, 120 + 192 * player -96 * maxPlay , 98  - (statusc[player * 10] - g_start[player]),  (statusc[player * 10] % 40) * 3, 28, 80, 8);
			ExBltRect(PLANE_LINE, 120 + 192 * player -96 * maxPlay , 126 + (statusc[player * 10] - g_start[player]),  160 - ((statusc[player * 10] % 40) * 3), 20, 80, 8);
			// GO
			ExBltRect(PLANE_TEXT2, 110+192 * player -96 * maxPlay , 91 , 200, 50, 100, 50);
		}
	}

	if(statusc[player * 10] == r_start[player]) PlaySE(WAVE_SE_READY);
	if(statusc[player * 10] == g_start[player]) PlaySE(WAVE_SE_GO);
	if(statusc[player * 10] >= g_end[player]) {
		// PRACTICEでエンディング中の場合#1.60c6.2d
		if((gameMode[0] == 5) && (ending[0] != 0)) {
			if(ending[0] == 1){
				AUDIO_PlayWave(WAVE_BGM_ENDING);						// BGM流れてなかったorz 差し替え#1.60c7i4
		...
		if(gameMode[player] == 4){	//VSのスタイル
			...
			if(statusc[player * 10] < g_start[player]){
				...
				ExBltRect(PLANE_TEXT,120 + 192 * player - 96 * maxPlay, 136, 240, 168, 80, 56);
				if(noitem)	//ITEMに×マーク
					ExBltRect(PLANE_HEBOSPR,127 + 192 * player - 96 * maxPlay, 166, 274, 384, 12, 12);
				if(getPushState(player, INPUT_BUTTON_DOWN)){
					PlaySE(WAVE_SE_MOVE);
					vs_style[player] = 0;	//NORMAL
				}else if(getPushState(player, INPUT_BUTTON_UP)){
					PlaySE(WAVE_SE_MOVE);
					vs_style[player] = 1;	//ATTACK
				}else if(getPushState(player, INPUT_BUTTON_RIGHT)){
					PlaySE(WAVE_SE_MOVE);
					vs_style[player] = 2;	//DEFENCE
				}else if((getPushState(player, INPUT_BUTTON_LEFT)) && (!noitem)){
					PlaySE(WAVE_SE_MOVE);
		...
		if( (gameMode[player] == 6) || ((gameMode[0] == 5) && (p_nextpass)) ) {
			if((!pass_flg[player]) && (getPressState(player, INPUT_BUTTON_D))) {
				PlaySE(WAVE_SE_HOLD);	// hold.wav
				// if not in FP-Basic
				if(!fpbas_mode[player])
				{
			...
			pass_flg[player] = getPressState(player, INPUT_BUTTON_D);
		}
	}
...
void statBlock(int32_t player) {
	...
	if((ishidenext[player] == 0) && (!isfakenext[player])){//HIDENEXTの時は音も隠す
		PlaySE(WAVE_SE_BLOCK1 + next[player]);
	}
	if(isfakenext[player]==1){//FAKENEXTの時は適当な音
		PlaySE(WAVE_SE_BLOCK1 + gameRand(6,player));
	}
...
void setGameOver(int32_t player) {
	...
	if( (gameMode[player] <= 3) || (gameMode[player] == 5) ) {
		...
		if( (sgrade[player] >= min_sgrade) && (gameMode[player] != 5) ) PlaySE(WAVE_SE_APPLAUSE);
	}
	...
	if((!((fastroll[player]) && (ending[player] == 2)) && !((gameMode[player] == 9) && (relaymode[player]) && (!ending[player]))) || (gameMode[player] == 5)){
...
void doIRS(int32_t player) {
	...
	if(repversw >= 43){	//新Ver
	if( getPressState(player, INPUT_BUTTON_A) || getPressState(player, INPUT_BUTTON_B) || getPressState(player, INPUT_BUTTON_C) ) {
		PlaySE(WAVE_SE_SHAKI);

		// 回転する方向を決める
		if( !getPressState(player, INPUT_BUTTON_B) ) {
			// AまたはC
		...
		// 180度回転
		if( ((rotspl[player] == 7) || (rotspl[player] == 8)) && getPressState(player, INPUT_BUTTON_C) ) {
			bak = 2;
		}
		...
		if( judgeBlock(player, bx[player], by[player], blk[player], bak) == 0 ) {
			// 回転可能
			if(rotate_snd) PlaySE(WAVE_SE_ROTATE);
			rt[player] = bak;
	...
	if(getPressState(player, INPUT_BUTTON_A) || getPressState(player, INPUT_BUTTON_C)) {
		PlaySE(WAVE_SE_SHAKI);
		...
		if( (isWRule(player)) && (w_reverse) )
			...
			(1*((rotspl[player] == 7) && getPressState(player, INPUT_BUTTON_C)))) == 0) +(1*((rotspl[player] == 7) && getPressState(player, INPUT_BUTTON_C)));
		else
			rt[player] = (judgeBlock(player, bx[player], by[player], blk[player], 3 -
			(1*((rotspl[player] == 8) && getPressState(player, INPUT_BUTTON_C)))) == 0) * 3 - (1*((rotspl[player] == 8) && getPressState(player, INPUT_BUTTON_C)));
		...
		if(!rt[player] && !blk[player] && !r_irs) {
			...
			if( (isWRule(player)) && (w_reverse) )
				...
				*((rotspl[player] == 7) && getPressState(player, INPUT_BUTTON_C)))) == 0) +(1*((rotspl[player] == 7) && getPressState(player, INPUT_BUTTON_C)));
			else
...
void doIRS2(int32_t player) {
	...
	// 回転方向逆転設定対応 激しく汚いorz #1.60c7f8
	if(getPushState(player, INPUT_BUTTON_A) || getPushState(player, INPUT_BUTTON_C)) {
		PlaySE(WAVE_SE_SHAKI);
		if(rotate_snd) PlaySE(WAVE_SE_ROTATE);	// 回転音設定#1.60c7f7
...
void doIRS2plus(int32_t player) {
	...
	if(getPressState(player, INPUT_BUTTON_A) || getPressState(player, INPUT_BUTTON_C)) {
		...
		if( (isWRule(player)) && (w_reverse) ){
			rt_nblk[0 + 6 * player]++;
			if((rotspl[player] == 7) && (getPressState(player, INPUT_BUTTON_C) != 0))
				rt_nblk[0 + 6 * player]++;
		}else{
			rt_nblk[0 + 6 * player]--;
			if((rotspl[player] == 8) && (getPressState(player, INPUT_BUTTON_C) != 0))
				rt_nblk[0 + 6 * player]--;
		}
		if(rt_nblk[0 + 6 * player] > 3) rt_nblk[0 + 6 * player] = rt_nblk[0 + 6 * player] - 4;
		if(rt_nblk[0 + 6 * player] < 0) rt_nblk[0 + 6 * player] = rt_nblk[0 + 6 * player] + 4;
...
void setBlockSpawnPosition(int32_t player) {
	...
	if(rotspl[player] == 8 && ((segacheat == 2) || (heboGB[player] != 2) || repversw < 66)){
		if(getPressState(player, INPUT_BUTTON_LEFT) && (judgeBlock(player, bx[player] - 1 - (1 * (IsBig[player] && BigMove[player])), by[player], blk[player], rt[player]) == 0)){
			bx[player] = bx[player] - 1 - (1 * (IsBig[player] && BigMove[player]));
			if(movesound) PlaySE(WAVE_SE_MOVE);
		}
		if((blk[player] != 0) || (!IsBig[player]))
		if(getPressState(player, INPUT_BUTTON_RIGHT) && (judgeBlock(player, bx[player] + 1 + (1 * (IsBig[player] && BigMove[player])), by[player], blk[player], rt[player]) == 0)){
			bx[player] = bx[player] + 1 + (1 * (IsBig[player] && BigMove[player]));
			if(movesound) PlaySE(WAVE_SE_MOVE);
		}
	}
...
void doHold(int32_t player, int32_t ihs) {
	...
	if(getPressState(player, INPUT_BUTTON_D) && !dhold[player]) {
	...
	if(statusc[player * 10 + 2] == 0){
		...
		if(statusc[player * 10 + 3] == 0){	//クリア演出
			...
			if( statusc[player * 10] > 22 ) {
				...
				if(block == 0) {
					PlaySE(WAVE_SE_REGRET);
					if((deadtype) || (heboGB[player])) PlaySE(WAVE_SE_TUMAGARI);	// 下から消去は音はなし #1.60c7h3
				}
		...
		if((getPushState(player, INPUT_BUTTON_A)) || (statusc[player * 10] > 600)){
			PlaySE(WAVE_SE_KETTEI);
			setNextBlockColors(player, 1);
			status[player] = 3;					// Ready
			statusc[player * 10] = 0;				// ステータスカウンタを0に
...
//	UpLineBlockJudge(player);
	...
	if((gameMode[player] == 6) && (!ending[player])) {
		...
		if( sclear[player] ) {
			thunder_timer = 0;
			PlaySE(WAVE_SE_STAGECLEAR);				// stageclear.wav
	...
	if(gameMode[player] == 8) {
		if((ltime[player] <= 0) && (mission_type[c_mission] == 23) && (!ending[player])){ //耐久ミッション
			missionNormUp(2);
			if( (ending[player] == 1) || (ending[player] == 4) ) PlaySE(WAVE_SE_TTCLEAR);
			statusc[player * 10 + 3] = 0;
		}
...
//		}
	...
	((getPushState(player, INPUT_BUTTON_UP)) || (getPushState(player, INPUT_BUTTON_DOWN)) || (getPushState(player, INPUT_BUTTON_LEFT)) ||
	 (getPushState(player, INPUT_BUTTON_RIGHT)) || (getPushState(player, INPUT_BUTTON_A)) || (getPushState(player, INPUT_BUTTON_B)) ||
	 (getPushState(player, INPUT_BUTTON_C)) || (getPushState(player, INPUT_BUTTON_D))) )
		statusc[player * 10] = -1;

	if( ((rotspl[player] == 8) && (are_skipflag[player])) &&
	((getPressState(player, INPUT_BUTTON_UP)) || (getPressState(player, INPUT_BUTTON_DOWN)) || (getPressState(player, INPUT_BUTTON_LEFT)) ||
	 (getPressState(player, INPUT_BUTTON_RIGHT)) || (getPressState(player, INPUT_BUTTON_A)) || (getPressState(player, INPUT_BUTTON_B)) ||
	 (getPressState(player, INPUT_BUTTON_C)) || (getPressState(player, INPUT_BUTTON_D))) ){
	 	are_skipflag[player] = 0;
		statusc[player * 10] = -1;
	}
...
//					AUDIO_PlayWave(WAVE_SE_UP);
					...
					if((fmirror_cnt[player] > 0) && (fmirror_cnt[player] % (p_fmirror_timer / fldsizew[player]) == 0)) {
						...
						ExBlt(PLANE_MIRROR_EFFECT_TAP, 120+ ((fldsizew[player] - j - 1) * 8) + 192 * player - 96 * maxPlay, 40);
					}
				}
				fmirror_cnt[player]++;
...
void statGameOver(int32_t player) {
	...
	if(gameMode[player] == 4) {
		...
		winu = - 24;
		wink = 0;
		if(AUDIO_IsWavePlaying(WAVE_BGM_FEVER)){
			AUDIO_StopWave(WAVE_BGM_FEVER);
			AUDIO_PlayWave(WAVE_BGM_START+bgmlv);
		}
	...
	if( statusc[player * 10] > 22 ) {
		if(statusc[player * 10 + 1] == 141) {
			//AUDIO_PlayWave(WAVE_SE_GAMEOVER);
...
//		statusc[player * 10]++;
		...
		// 1ライン以上消えた場合
			...
			if(hebocombo[player]) PlaySE(WAVE_SE_CHEER);
			combo[player]++;
			...
			if(ismiss[player]) PlaySE(WAVE_SE_REGRET);
	...
	if(statusc[player * 10] >= wait2[player] + (repversw < 27)) {
		...
		//エンディング突入音 #1.60c7j9
		if( (ending[player] == 1) || (ending[player] == 4) ||(ending[player] >= 6)) PlaySE(WAVE_SE_TTCLEAR);
		status[player] = 6;
		statusc[player * 10] = wait1[player];
		statusc[player * 10 + 1] = 15;
		if(((gameMode[player] == 9) && (relaymode[player])) || (gameMode[player] == 5)) StopSE(WAVE_SE_TTCLEAR);
...
void checkEnding(int32_t player, int32_t tcbuf) {
	...
	// BEGINNERなら200で終了させる#1.60c7n2
		...
		//AUDIO_PlayWave(WAVE_SE_TTCLEAR);
		tc[player] = 200;
		lv[player] = tc[player];
		timeOn[player] = 0;
		ending[player] = 1;
		...
		//AUDIO_PlayWave(WAVE_SE_LEVELUP);
		if(backno <= 1) {
			bgfadesw = 1;
		}
	// NOVICEなら300で終了させる
	...
	// DEVILなら1300で終了させる#1.60c7f3
		...
		if(( ((!isWRule(player)) && (gametime[player] <= 19200)) || ((isWRule(player)) && (gametime[player] <= 21000)) )&&(MAIN_GetRealFPS()>40)) {
			grade[player] = 16;
			objectCreate2(player, 8, SDL_rand(20) + 180 + 192 * player - 96 * maxPlay, 20 + SDL_rand(10), 0, 0, 0, 0);
			AUDIO_PlayWave(WAVE_SE_STGSTAR);
...
void statVersusWait(int32_t player) {
	...
	if(gameMode[0] == 4) {
		if(count % 60 > 30) {
			ExBltRect(PLANE_TEXT2, 110+192 * player -96 * maxPlay , 91 , 100, 0, 100, 50);
		}
...
void statNameEntry(int32_t player) {
	...
	if( ((status[1 - player] == 0) || (status[1 - player] == 10)) && (!AUDIO_IsWavePlaying(WAVE_BGM_NAMEENTRY)) && !(wavebgm & WAVE_BGM_SIMPLE) )
		AUDIO_PlayWave(WAVE_BGM_NAMEENTRY);
/*
	// リプレイセーブ#1.60c7i5
...
*/
	...
	if((enable_grade[player] == 4) && ((gameMode[player] == 1) || (gameMode[player] == 2))) {
		ExBltRect(PLANE_HEBOSPR, 120 + 192 * player - 96 * maxPlay, 137 - (add * 3), 228, 119, 26, 7);

		if(grade[player] == 32) {
			// Gm
				ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 144 - (add * 3), 270 ,((count % 4 / 2)*216)+120,90,48);
		}else if( grade[player] >= 28 ) {
			// MK-MM
			ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 144 - (add * 3), 270 ,((count % 4 / 2)*216)+((grade[player]-27)*24),90,24);
		}  else if( grade[player] == 27 ) {
			//Master
			ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 144 - (add * 3), 270 ,((count % 4 / 2)*216),90,24);
		} else if( grade[player] >= 18 ) {
			// m1〜m9
			ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 144 - (add * 3), 180 ,((count % 4 / 2)*216)+((grade[player]-18)*24),90,24);
		} else if( grade[player] >= 9 ) {
			// S1〜S9
			ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 144 - (add * 3), 90 ,((count % 4 / 2)*216)+((grade[player]-9)*24),90,24);
		} else {
			// 9〜1
			ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 144 - (add * 3), 0 ,((count % 4 / 2)*216)+(grade[player]*24),90,24);
		}
	}
	// S-GRADE #1.60c7o1
	if( sgrade[player] >= min_sgrade ) {
		ExBltRect(PLANE_HEBOSPR, 120 + 192 * player - 96 * maxPlay, 160, 220, 91, 31, 7);
		ExBltRect(PLANE_HEBOSPR, 152 + 192 * player - 96 * maxPlay, 160, 228, 119, 26, 7);
		if( sgrade[player] == 18 ) {
			// GM
			ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 168, 270 ,((count % 4 / 2)*216)+168,90,24);
		} else if( sgrade[player] >= 9 ) {
			// S1〜S9
			ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 168, 90 ,((count % 4 / 2)*216)+((sgrade[player]-9)*24),90,24);
		} else {
			// 9〜1
			ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 168, 0 ,((count % 4 / 2)*216)+(sgrade[player]*24),90,24);
		}
	}
	...
	if(statusc[player * 10 + 1] < 45 * 60){
		...
		move = getPressState(player, INPUT_BUTTON_RIGHT) - getPressState(player, INPUT_BUTTON_LEFT);
		if(move)
			...
			if(ending[player] ==1) {
				...
				if(gameMode[player] >= 6) {
					...
					if((gameMode[player] == 6) && (stage[player] == 44) && (!t_training[player])){
						...
						ending[player] = 2;
						// エンディングBGM再生
						AUDIO_PlayWave(WAVE_BGM_ENDING);
						fadelv[player] = 0;
					}
...
fadelv[player] = 0;
					...
					if ((gameMode[player] == 0) && (beginner_rollbgm == 0))
						AUDIO_PlayWave(WAVE_BGM_ENDING_B);
					else if ((gameMode[player] == 0) && (beginner_rollbgm == 1))
						AUDIO_PlayWave(WAVE_BGM_BGM02);
					else
						AUDIO_PlayWave(WAVE_BGM_ENDING);
...
fadelv[player] = 0;
	...
	else if ( ending[player] == 3 )
	{
		...
		if(gameMode[player] == 8) {
			// ミッションモード用
			ExBltRect(PLANE_HEBOSPR, 105 + 192 * player - 96 * maxPlay, 76, 195, 230, 109, 20);
			...
			if(statusc[player * 10] < 64) {
				...
				j = (36 * k / 64 - 36) / 2;
				k = 65536 * k / 64;
				ExBltRectScaled(PLANE_HEBOSPR, 120 - i + 192 * player - 96 * maxPlay, 102 - j, 208, 126, 80, 36, k, k);
			}
			else{
				ExBltRect(PLANE_HEBOSPR, 120 + 192 * player - 96 * maxPlay, 102, 208, 126, 80, 36);
			}
			...
			if(statusc[player * 10] < 64) {
				...
				j = (66 * k / 64 - 66) / 2;
				k = 65536 * k / 64;
				ExBltRectScaled(PLANE_HEBOSPR, 105 - i + 192 * player - 96 * maxPlay, 70 - j, 191, 163, 112, 66, k, k);
			}
			else{
				ExBltRect(PLANE_HEBOSPR, 105 + 192 * player - 96 * maxPlay, 70, 191, 163, 112, 66);
			}
		} else {
			...
			if(statusc[player * 10] <= 5){
				...
				j = (9 * k / 5 - 9) / 2;
				k = 65536 * k / 5;
				ExBltRectScaled(PLANE_HEBOSPR, 122 - i + 192 * player - 96 * maxPlay, 96 - j, 212, 73, 75, 9, k, k);
			} else {
				ExBltRect(PLANE_HEBOSPR, 122 + 192 * player - 96 * maxPlay, 96, 212, 73, 75, 9);
			}

			// DEVIL-
			if((gameMode[player] == 3) && (devil_minus[player]==1)){
				printSMALLFont(130 + 192 * player - 96 * maxPlay, 157, "Try Next", 0);
				ExBltRect(PLANE_GAMEMODEFONT, 130 + 192 * player - 96 * maxPlay, 166, 0, 27, 72, 9);	// DEVIL-DOOM
				printSMALLFont(130 + 192 * player - 96 * maxPlay, 175, "   Mode", 0);
			}
			// ゲームモードの表示
			ExBltRect(PLANE_GAMEMODEFONT, 125 + 192 * player - 96 * maxPlay, 110, 72*getModeAdd(player), 99 + (9*gameMode[player]), 72, 9);
			...
			if(((gameMode[player] == 1) || (gameMode[player] == 2)) && (enable_grade[player]!=4)  && (examination[player]==0)){
				ExBltRect(PLANE_HEBOFONT5, 126 + 192 * player - 96 * maxPlay, 157, 0,15*7, 64, 14);//TRY AGAIN云々の文字画像
			}
		}
	...
	else if ( ending[player] == 5 )		// とりカンを追加1.60c7g7
	{
		...
		if(statusc[player * 10] < 220) {
			if(gameMode[player] == 8)
				ExBltRect(PLANE_HEBOSPR, 105 + 192 * player - 96 * maxPlay, 76, 195, 250, 109, 20);
			else{
				// EXCELLENT
				if(statusc[player * 10] <= 5){
					k = 10 - statusc[player * 10];
...
*/
	...
	if((gameMode[player] == 9) && (relaymode[player])){	// ROTリレー
		...
		if((gameMode[player] != 0)&& (gameMode[player] < 6)) {
			ExBltRect(PLANE_HEBOSPR, 120 + 192 * player - 96 * maxPlay, 125, 228, 119, 26, 7);
			...
			if(gameMode[player] != 3) {
				...
				if(grade[player] == 32) {
					if(enable_grade[player]==1){
					//GM
						ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 135, 443 ,((count % 4 / 2)*216)+180,90,32);
					}else{
					// Gm
						ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 135, 270 ,((count % 4 / 2)*216)+120,90,48);
					}
				}else if( grade[player] >= 28 ) {
					// MK-MM
					ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 135, 270 ,((count % 4 / 2)*216)+((grade[player]-27)*24),90,24);
				}  else if( grade[player] == 27 ) {
					if((enable_grade[player] == 2)||(enable_grade[player] == 4)){
						//Master
						ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 135, 270 ,((count % 4 / 2)*216),90,24);
					}else{
						//m
						ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 135,443 ,((count % 4 / 2)*216),90,24);
					}
				} else if( grade[player] >= 18 ) {
					// m1〜m9
					ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 135, 180 ,((count % 4 / 2)*216)+((grade[player]-18)*24),90,24);
				} else if( grade[player] >= 9 ) {
					// S1〜S9
					ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 135, 90 ,((count % 4 / 2)*216)+((grade[player]-9)*24),90,24);
				} else {
					// 9〜1
					ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 135, 0 ,((count % 4 / 2)*216)+(grade[player]*24),90,24);
				}
			} else {
				// DEVIL
				if( grade[player] == 0 ) {
					// 1
					ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 135, 0 ,((count % 4 / 2)*216)+192,90,24);
				} else {
					if(grade[player] < 10) { /* S1〜S9 */
						ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 135, 90 ,((count % 4 / 2)*216)+((grade[player]-1)*24),90,24);
					} else if(grade[player] < 14)  { /* S10〜S13 */
						ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 135, 360 ,((count % 4 / 2)*216)+((grade[player]-10)*24),90,24);
					} else if(grade[player] == 14) { /* m */
						ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 135,443 ,((count % 4 / 2)*216),90,24);
					} else if(grade[player] == 15) { /* Gm */
						ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 135, 270 ,((count % 4 / 2)*216)+120,90,48);
					} else if(grade[player] == 16) { /* ネ申 */
						ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 130, 360 ,((count % 4 / 2)*216)+96,90,48);
					}
				}
			}
		}
		...
		if(gameMode[player] == 0){
			if(!novice_mode[player]){
				...
				ExBltRect(PLANE_HEBOSPR, 120 + 192 * player - 96 * maxPlay, 125, 154, 112, 26, 7);
...
void statReplaySave(int32_t player) {
	...
	if((time2[player] > REPLAY_TIME_MAX) || (!replay_save[player])){	//リプレイ記録不可
		...
		ExBltRect(PLANE_HEBOFONT5, 120 + 192 * player - 96 * maxPlay, 77, 0, 98, 22, 7);	//モード
		ExBltRect(PLANE_GAMEMODEFONT, 125 + 192 * player - 96 * maxPlay, 87, 72*repdata[1 + player * 20], 99 + (9*repdata[0 + player * 20]), 72, 9);

		ExBltRect(PLANE_HEBOFONT5, 120 + 192 * player - 96 * maxPlay, 105, 35, 42, 22, 7);	//Rots
		if(repdata[2 + player * 20] != 9)
			ExBltRect(PLANE_ROT,128 + 8 * (repdata[0 + player * 20] == 4) + 192 * player -96 * maxPlay , 113 - (4 * (getDrawRate() == 1)), 64*repdata[2 + player * 20] ,
		...
		if(repdata[0 + player * 20] == 6){	//TOMOYO
			ExBltRect(PLANE_HEBOSPR, 120 + 192 * player - 96 * maxPlay, 131, 228, 112, 26, 7);
			SDL_snprintf(string[0], STRING_LENGTH, "%6d", repdata[4 + player * 20] + 1);
			printSMALLFont(158 + 192 * player - 96 * maxPlay, 131, string[0], 0);

			ExBltRect(PLANE_HEBOSPR, 120 + 192 * player - 96 * maxPlay, 147, 180, 119, 19, 7);
			...
			if(repdata[4 + player * 20] != 9)
				ExBltRect(PLANE_ROT,128 + 8 + 192 * player -96 * maxPlay , 137 - (4 * (getDrawRate() == 1)), 64*repdata[4 + player * 20] ,
			...
			ExBltRect(PLANE_HEBOFONT5, (18 + 24 * player - 12 * maxPlay)*8, 96, 0, 7*(25 + repdata[4 + player * 20]), 50, 7);
			if(repdata[4 + player * 20] > 1){
				ExBltRect(PLANE_HEBOSPR, 120 + 192 * player - 96 * maxPlay, 131, 154, 112, 26, 7);
				SDL_snprintf(string[0], STRING_LENGTH, "%6d", repdata[5 + player * 20]);
				printSMALLFont(158 + 192 * player - 96 * maxPlay, 131, string[0], 0);
			}
			...
			ExBltRect(PLANE_HEBOSPR, 120 + 192 * player - 96 * maxPlay, 163, 154, 112, 26, 7);
			SDL_snprintf(string[0], STRING_LENGTH, "%6d", repdata[6 + player * 20]);
			printSMALLFont(158 + 192 * player - 96 * maxPlay, 163, string[0], 0);

			ExBltRect(PLANE_HEBOSPR, 120 + 192 * player - 96 * maxPlay, 179, 154, 119, 23, 7);
			SDL_snprintf(string[0], STRING_LENGTH, "%6d", repdata[7 + player * 20]);
			printSMALLFont(158 + 192 * player - 96 * maxPlay, 179, string[0], 0);
		}
		if(repdata[9 + player * 20]) ExBltRect(PLANE_HEBOFONT5, 136 + 192 * player - 96 * maxPlay, 96, 154, 112, 14, 7);
	}
	if(statusc[player * 10 + 2] == 0){
		printMenuButton(16+24 * player - 12 * maxPlay, 25, INPUT_BUTTON_B, player);
		...
		if( getPushState(player, INPUT_BUTTON_A) ) {
			if((time2[player] > REPLAY_TIME_MAX) || (!replay_save[player])){
				...
				if(SDL_abs(MAIN_GetRealFPS() - max_fps_2) < 10){
					PlaySE(WAVE_SE_KETTEI);
...
void statVersusSelect(int32_t player) {
	...
	if(vslevel[player] == 2) {
		printMenuButton(3 + 24 * player, 6, INPUT_BUTTON_A, player);
	...
	if((vsmodesl==0)||(vsmodesl==3)){
		...
		if(use_item[player] == 0){
			...
			ExBltRect(PLANE_HEBOBLK_SP,(12 + 24 * player)*8, 136,(8*(use_item[player]-1))+56,0,8,8);
			ExBltRect(PLANE_ITEM,(7 + 24 * player)*8, 144, 0, (use_item[player] - 1) * 16, 48, 16);
		}else if(use_item[player] == item_num+1){
			printFont(10 + 24 * player, 17, "FEW", count % 9 * (vslevel[player] == 8));
			ExBltRect(PLANE_HEBOBLK_SP,(3 + 24 * player)*8, 144,56,0,40,8);
		}else if(use_item[player] == item_num+2){
			printFont(10 + 24 * player, 17, "DS", count % 9 * (vslevel[player] == 8));
			ExBltRect(PLANE_HEBOBLK_SP,(3 + 24 * player)*8, 144,(8*5)+56,0,8,8);
			ExBltRect(PLANE_HEBOBLK_SP,(4 + 24 * player)*8, 144,(8*6)+56,0,8,8);
			ExBltRect(PLANE_HEBOBLK_SP,(5 + 24 * player)*8, 144,(8*11)+56,0,8,8);
			ExBltRect(PLANE_HEBOBLK_SP,(6 + 24 * player)*8, 144,(8*12)+56,0,8,8);
			ExBltRect(PLANE_HEBOBLK_SP,(7 + 24 * player)*8, 144,(8*17)+56,0,8,8);
			ExBltRect(PLANE_HEBOBLK_SP,(8 + 24 * player)*8, 144,(8*25)+56,0,8,8);
		}else if(use_item[player] == item_num+3){
			printFont(10 + 24 * player, 17, "TGM", count % 9 * (vslevel[player] == 8));
			ExBltRect(PLANE_HEBOBLK_SP,(3 + 24 * player)*8, 144,56,0,40,8);
			ExBltRect(PLANE_HEBOBLK_SP,(8 + 24 * player)*8, 144,176,0,40,8);
			ExBltRect(PLANE_HEBOBLK_SP,(3 + 24 * player)*8, 152,216,0,40,8);
			ExBltRect(PLANE_HEBOBLK_SP,(8 + 24 * player)*8, 152,272,0,32,8);
		}
	}else{
	...
	if( (mpc2[player] == 1) || ((mpc2[player] > tame3) && (mpc2[player] % tame4 == 0)) ) {
		if(getPressState(player, INPUT_BUTTON_UP)) {
			...
			if(getPressState(player, INPUT_BUTTON_D)){
				winpoint--;
...
//	pinch[1] = 0;
	...
	for(player = 0; player <= 1; player++) {
		...
		ExBltRect(PLANE_HEBOSPR, 22 + player * 192, winu, 96, obji * 25, 86, 24);
		...
		// Aボタンで設定画面に戻る
		if((getPushState(player, INPUT_BUTTON_A) && (winr > 22) && (!playback)) || ((demo) && (winr > 19)) || (winr > 80)) {
			if((playback) || (demo)){
				if(!tmp_maxPlay)
					maxPlay = 0;
	...
	if(winc > 4) {
		...
		// 下から消去は音なし #1.60c7h3
		if((winr == 0) && (deadtype)) PlaySE(WAVE_SE_TUMAGARI);
...
//	gametime[1] = gametime[0];
	...
	for(player = 0; player <= 1; player++) {
		...
		if(obji == 2) ExBltRect(PLANE_HEBOSPR, 22 + player * 192, winu, 96, obji * 25, 86, 24);
...
void eraseItem(int32_t player, int32_t type) {
	...
	// 解除ここまで
		if(!ending[player]){	//ロール中はBGMを変えない
			AUDIO_StopWave(WAVE_BGM_START+bgmlv);
			AUDIO_PlayWave(WAVE_BGM_FEVER);
		}
		isfever[player] = 1;
...
void statGameOver2(int32_t player) {
	...
	*/
	// ゲームオーバー音
	if(statusc[player * 10] == 1) PlaySE(WAVE_SE_GAMEOVER);
	if(statusc[player * 10] >= 2) {
		ExBltRect(PLANE_TEXT2, 110+192 * player -96 * maxPlay , 95 , 0, 0, 100, 50);
	}
		...
		if(statusc[player * 10 + 2] == 0){//レーザー
			if(statusc[player * 10 + 0] < waitA){	//レーザー発射前
				if(statusc[player * 10 + 0] % 5 == 0)
					PlaySE(WAVE_SE_KACHI);
				if( ( getPushState(player, INPUT_BUTTON_A)) || ( getPushState(player, INPUT_BUTTON_B)) || ( getPushState(player, INPUT_BUTTON_C)) )
					rapid_c[player]++;
				...
				//照準を移動
				// →	照準は動かしにくくする
				if((getPushState(player, INPUT_BUTTON_RIGHT)) && (statusc[player * 10 + 0] % 10 < 2)){
					for(i = 0; i < lasernum[player]; i++){
						if(laserpos[i + 4 * player] < fldsizew[player] - 1) laserpos[i + 4 * player]++;
					}
				}
				// ←	移動入力の受付は一定間隔で
				if((getPushState(player, INPUT_BUTTON_LEFT)) && (statusc[player * 10 + 0] % 10 < 2)){
					for(i = 0; i < lasernum[player]; i++){
				...
				// 照準を描画
				for(i = 0; i < lasernum[player]; i++){
					ExBltRect(PLANE_LASER, ((laserpos[i + 4 * player] + 15 + 24 * player - 12 * maxPlay) - 1) * 8, (4 - 1) * 8, 0, ((statusc[player * 10 + 0] % 36) / 6) * 24, 24, 24);
				}
			} else if(statusc[player * 10 + 0] == waitA){	// レーザー発射
				PlaySE(WAVE_SE_ERASE2);
				for(i = 0; i < lasernum[player]; i++){	// 照準直下のブロックを消去
					ExBltRect(PLANE_LASER, (laserpos[i + 4 * player] + 15 + 24 * player - 12 * maxPlay) * 8, 5 * 8, 24, 0, 8, 160);
					ExBltRect(PLANE_LASER, ((laserpos[i + 4 * player] + 15 + 24 * player - 12 * maxPlay) - 1) * 8, (4 - 1) * 8, 0, 0, 24, 24);
				...
				for(i = 0; i < lasernum[player]; i++){
					if(statusc[player * 10 + 0] - waitA < 34){
						...
						ExBltRect(PLANE_LASER, (laserpos[i + 4 * player] + 15 + 24 * player - 12 * maxPlay) * 8, 5 * 8, 24+(j*8), 0, 8, 160);
					}
					ExBltRect(PLANE_LASER, ((laserpos[i + 4 * player] + 15 + 24 * player - 12 * maxPlay) - 1) * 8, (4 - 1) * 8, 0, 0, 24, 24);
				}
			}
...
//					for(j = 0; j <= fldsizeh[player]; j++){
					...
					if((j - 1 >= 0) && (j - 1 <= fldsizeh[player])){
						...
						//	objectCreate(player, 1, (laserpos[i + 4 * player] + 15 + 24 * player - 12 * maxPlay) * 8, (j + 2) * 8, (laserpos[i + 4 * player] - 5) * 120 + 20 - SDL_rand(40), - 1900 + SDL_rand(150), fld[laserpos[i + 4 * player] + j * 10 + player * 220], 100);
			...
			//	j = statusc[player * 10 + 0] - waitA;
				ExBltRect(PLANE_HEBOSPR, ((laserpos[0 + 4 * player] + 15 + 24 * player - 12 * maxPlay) - 1) * 8, ((j + 3)*8) - 20, 260, 399, 40, 21);
			}
		}
...
void statExchangefield(int32_t player) {
	...
	if(statusc[player * 10 + 0] == 0){	//最初
		...
		ExBlt(PLANE_MIRROR_EFFECT_TAP, 120+ ((statusc[player * 10 + 0] - 60) * 8) + 192 * player - 96 * maxPlay, 40);
		scanItem(player);
		}
	...
	if(statusc[player * 10 + 1] == 0){		//開始宣言
		if(statusc[player * 10 + 0] == 0){//プルプル
			...
			if(statusc[player * 10 + 0] < 170)
				ExBltRectScaled(PLANE_TEXT2, 110 - ((100 * 22 / 12 - 100) / 2) +192 * player -96 * maxPlay , 80 , 200, 100 + 25 * (count % 4 / 2), 100, 25,65536 * 22 / 12,65536 * 22 / 12);
				...
				ExBltRectScaled(PLANE_TEXT2, 110 - i + 192 * player -96 * maxPlay , 48 + 192 - statusc[player * 10 + 0] , 200, 100 + 25 * (count % 4 / 2), 100, 25,k,k);
			}else
				ExBltRect(PLANE_TEXT2, 110+192 * player -96 * maxPlay , 48 , 200, 100 + 25 * (count % 4 / 2), 100, 25);
			if(statusc[player * 10 + 0] > 210){		//目標段位の表示
				ExBltRect(PLANE_TEXT2, 110+192 * player -96 * maxPlay , 74 , 200, 180, 100, 20);
				...
				if(exam_grade[player]== 32) {
					// GM
					ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 115, 270 ,((count % 4 / 2)*216)+120,90,48);
				} else if( exam_grade[player] >= 28 ) {
					// MK-MM
					ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 127, 270 ,((count % 4 / 2)*216)+((exam_grade[player]-27)*24),90,24);
					...
					if((enable_grade[player]==2)||(enable_grade[player]==4)){
						...
						// m
						ExBltRect(PLANE_GRADE,124 + 192 * player -96 * maxPlay , 127,450 ,((count % 4 / 2)*216)+((exam_grade[player]-27)*24),90,24);
					}
				} else if( exam_grade[player] >= 18 ) {
					// m1〜m9
					ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 127, 180 ,((count % 4 / 2)*216)+((exam_grade[player]-18)*24),90,24);
				} else if( exam_grade[player] >= 9 ) {
					// S1〜S9
					ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 127, 90 ,((count % 4 / 2)*216)+((exam_grade[player]-9)*24),90,24);
				} else {
					// 9〜1
					ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 127, 0 ,((count % 4 / 2)*216)+(exam_grade[player]*24),90,24);
				}
			}
		...
		if(statusc[player * 10 + 0] == 0){
			...
			ExBltRect(PLANE_TEXT2, 110+192 * player -96 * maxPlay , 48 , 200, 150, 100, 30);
			...
			if(statusc[player * 10 + 0] > 90){		//目標段位の表示
				...
				if(exam_grade[player] == 32) {
					...
					if((enable_grade[player]==2)||(enable_grade[player]==4)){
						...
						// m
						ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 127,450 ,((count % 4 / 2)*216)+((exam_grade[player]-27)*24),90,24);
					}
				}else if( exam_grade[player] >= 18 ) {
					// m1〜m9
					ExBltRect(PLANE_GRADE,115 + 192 * player -96 * maxPlay , 127, 180 ,((count % 4 / 2)*216)+((exam_grade[player]-18)*24),90,24);
			...
			if((statusc[player * 10 + 0] > 150) && (statusc[player * 10 + 0] < 350)){	//ルーレット
				...
				if((statusc[player * 10 + 0] % 10 < 5) || (statusc[player * 10 + 0] >= 320)){
					ExBltRect(PLANE_TEXT2, 125+192 * player -96 * maxPlay , 162 , 215, 250, 70, 25);
					printSMALLFont(125 + 192 * player - 96 * maxPlay, 189, "   FAILED   ",0);
				}else{
					ExBltRect(PLANE_TEXT2, 125+192 * player -96 * maxPlay , 162 , 215, 200, 70, 25);
			...
			if(statusc[player * 10 + 0] >= 350){
				if(exam_grade[player] <= grade[player]){	//合格
					...
					ExBltRect(PLANE_TEXT2, 125+192 * player -96 * maxPlay , 162 , 215, 200 + 25 * (count % 4 / 2), 70, 25);
					printSMALLFont(122 + 192 * player - 96 * maxPlay, 189, "   SUCCESS   ",(count % 4 / 2) * digitc[rotspl[player]]);
				}else{										//不合格
					ExBltRect(PLANE_TEXT2, 125+192 * player -96 * maxPlay , 162 , 215, 250 + 25 * (count % 4 / 2), 70, 25);
...
void statItemRulet(int32_t player) {
	...
	if(statusc[player * 10 + 0] == 0){	//最初
		...
		if(statusc[player * 10 + 0] <= 150)ExBltRect(PLANE_ITEMGRA, 120+192 * player -96 * maxPlay , 142 , 80 * ((statusc[player * 10 + 2] - 1) / 10), 36 * ((statusc[player * 10 + 2] - 1) % 10), 80, 36);
		ExBltRect(PLANE_HEBOBLK_SP,(4 + 24 * player + 12 * (!maxPlay))*8, 182,(8*(statusc[player * 10 + 2] - 1))+56,0,8,8);
		ExBltRect(PLANE_HEBOBLK_SP,(11 + 24 * player + 12 * (!maxPlay))*8, 182,(8*(statusc[player * 10 + 2] - 1))+56,0,8,8);
		ExBltRect(PLANE_ITEM,(5 + 24 * player + 12 * (!maxPlay))*8, 178, 0, (statusc[player * 10 + 2] - 1) * 16, 48, 16);
		statusc[player * 10 + 0]++;
		if(statusc[player * 10 + 0] >= waitA){
			if(repversw >= 48){
...
void statFreefall(int32_t player) {
	...
	if(statusc[player * 10 + 0] == (wait1[player] * 2) + 30){	//消去判定（なければ次のステータスへ）
		if(blockEraseJudgeFf(player,0) == 0){	// 消去なし
			...
			if(gameMode[player] != 4)
				PlaySE(WAVE_SE_ERASE1);
			else if( Ff_rerise[player] >= 5 )
				PlaySE(WAVE_SE_ERASE1-1 + 4);		// 5ライン以上の場合は4ラインの音を出す
			else
				PlaySE(WAVE_SE_ERASE1-1 + Ff_rerise[player]);
			if(gameMode[player] == 4){
...
void statBanana(int32_t player){
	...
	if(banana_c[player] == 0){	// 最初
		...
		if((y > 1) && (y <= 7))
			ExBltRect(PLANE_SHUFFLE_FIELD_EFFECT, 120 + 192 * player - 96 * maxPlay, 32, 0 , 50 * (count % 4) + (50 - ((y - 1) * 8)), 80, (y - 1) * 8);
		else if((y > 21) && (y <= 27))
			ExBltRect(PLANE_SHUFFLE_FIELD_EFFECT, 120 + 192 * player - 96 * maxPlay, 26 + (y * 8) - 50, 0 , 50 * (count % 4), 80, ((28 - y) * 8));
		else if((y > 7) && (y <= 21))
			ExBltRect(PLANE_SHUFFLE_FIELD_EFFECT, 120 + 192 * player - 96 * maxPlay, 24 + (y * 8) - 50, 0 , 50 * (count % 4), 80, 50);
		banana_c[player]++;
	}
...
void padRepeat(int32_t player) {
	if(getPressState(player, INPUT_BUTTON_LEFT) && getPressState(player, INPUT_BUTTON_RIGHT)) {
		mp[player] = 0;
		mpc[player] = 0;
	} else if(getPressState(player, INPUT_BUTTON_LEFT)) {
		if(mp[player] != 2) {
			mp[player] = 2;
			mpc[player] = 0;
		}
		mpc[player]++;
	} else if(getPressState(player, INPUT_BUTTON_RIGHT)) {
		if(mp[player] != 3) {
			mp[player] = 3;
			mpc[player] = 0;
		}
...
void padRepeat2(int32_t player) { // hoge 上下入力バージョン
	if(getPressState(player, INPUT_BUTTON_UP) && getPressState(player, INPUT_BUTTON_DOWN)) {
		mp2[player] = 0;
		mpc2[player] = 0;
	} else if(getPressState(player, INPUT_BUTTON_UP)) {
		if(mp2[player] != 1) {
			mp2[player] = 1;
			mpc2[player] = 0;
		}
		mpc2[player]++;
	} else if(getPressState(player, INPUT_BUTTON_DOWN)) {
		if(mp2[player] != 2) {
			mp2[player] = 2;
			mpc2[player] = 0;
		}
...
void nextFourWayFilteredDirection(
	...
	if (fourwaypriorityup == 0 || fourwaypriorityup == player + 1) {
		...
		if (nextUp) {
			*direction = INPUT_BUTTON_UP;
		}
		else if (nextDown) {
			*direction = INPUT_BUTTON_DOWN;
		}
		else if (nextLeft) {
			*direction = INPUT_BUTTON_LEFT;
		}
		else if (nextRight) {
			*direction = INPUT_BUTTON_RIGHT;
		}
		else {
			*direction = INPUT_BUTTON_NULL;
		}
	}
	else {
...
int32_t getPressState(int32_t player, INPUT_Button key) { // パッドボタン割り当て対応 #1.60c
	...
	// 通常プレイ中
	...
	else if ((fourwayfilter == 0 || fourwayfilter == pl + 1) && INPUT_BUTTON_IS_DIR(key)) {
		return pressDirection[pl] == key;
	}
	else {
		#ifdef INPUT_ENABLE_LINUX_GPIO
		if ( player == 0 ) {
			gtmp = INPUT_IsPressGPIO(key);
		}
		else
			gtmp = 0;
		#endif
		...
		#ifdef INPUT_ENABLE_KEYBOARD
		// キーボードの入力を読み取る
		ktmp = INPUT_IsPressKey(keyAssign[key + pl * 10]);
		#endif

		#ifdef INPUT_ENABLE_JOYSTICK
		// ジョイスティックの入力を読み取る
		jtmp = INPUT_IsPressJoyKey(&joyKeyAssign[key + pl * 10]);
		#endif

		#ifdef INPUT_ENABLE_GAME_CONTROLLER
		INPUT_ConKey conkey;
		switch (key) {
		case INPUT_BUTTON_GIVE_UP:
			conkey.type = INPUT_CON_KEY_BUTTON;
			conkey.index = SDL_GAMEPAD_BUTTON_BACK;
			ctmp = INPUT_IsPressConKey(pl, &conkey);
			break;

		case INPUT_BUTTON_PAUSE:
			conkey.type = INPUT_CON_KEY_BUTTON;
		...
		default:
			ctmp = inmenu ?
				IsPressMenu(pl, key, INPUT_GetConType(pl)) :
				INPUT_IsPressConKey(pl, &conKeyAssign[key + 8 * pl]);
			break;
		}
		#endif
...
int32_t getPushState(int32_t player, INPUT_Button key) { // パッドボタン割り当て対応 #1.60c
	#ifdef INPUT_ENABLE_LINUX_GPIO
	int32_t gtmp;		// GPIO入力
	#endif
	#ifdef INPUT_ENABLE_JOYSTICK
	...
	if(playback && replayData) {
		// 入力をリプレイデータから読み取る
		ktmp = (replayData[time2[pl] / REPLAY_PLAYER_CHUNK][(time2[pl] % REPLAY_PLAYER_CHUNK) + pl * REPLAY_PLAYER_CHUNK] & (1 << (key + INPUT_BUTTON_GAME_COUNT))) / (1 << (key + INPUT_BUTTON_GAME_COUNT));
	...
	// 通常プレイ中
		...
		#ifdef INPUT_ENABLE_KEYBOARD
		// キーボードの入力を読み取る
		ktmp = INPUT_IsPushKey(keyAssign[key + pl * 10]);
		#endif

		#ifdef INPUT_ENABLE_JOYSTICK
		// ジョイスティックの入力を読み取る
		jtmp = INPUT_IsPushJoyKey(&joyKeyAssign[key + 10 * pl]);
		#endif
		...
		case INPUT_BUTTON_GIVE_UP:
			conkey.type = INPUT_CON_KEY_BUTTON;
			conkey.index = SDL_GAMEPAD_BUTTON_BACK;
			ctmp = INPUT_IsPushConKey(pl, &conkey);
			break;
		...
		default:
			ctmp = inmenu ?
				IsPushMenu(pl, key, INPUT_GetConType(pl)) :
				INPUT_IsPushConKey(pl, &conKeyAssign[key + 8 * pl]);
			break;
		}
		#endif
...
int IsPressMenu(int32_t player, INPUT_Button button, INPUT_DeviceType type)
{
	#ifdef INPUT_ENABLE_GAME_CONTROLLER
	INPUT_ConKey key;
	int pushed;
	#endif
	#ifdef INPUT_ONLY_GAME_CONTROLLER_TYPE
	switch (INPUT_ONLY_GAME_CONTROLLER_TYPE)
	#else
	switch (type)
	#endif
	{
	#ifdef INPUT_ENABLE_GAME_CONTROLLER
	case INPUT_DEVICE_TYPE_XBOX:
	case INPUT_DEVICE_TYPE_PLAYSTATION:
	case INPUT_DEVICE_TYPE_NINTENDO:
		switch (button)
		{
		case INPUT_BUTTON_UP:
			key = (INPUT_ConKey) { .type = INPUT_CON_KEY_AXIS, .index = 3 };
			pushed = INPUT_IsPressConKey(player, &key);
			key = (INPUT_ConKey) { .type = INPUT_CON_KEY_BUTTON, .index = SDL_GAMEPAD_BUTTON_DPAD_UP };
			return pushed || INPUT_IsPressConKey(player, &key);
		case INPUT_BUTTON_DOWN:
			key = (INPUT_ConKey) { .type = INPUT_CON_KEY_AXIS, .index = 2 };
			pushed = INPUT_IsPressConKey(player, &key);
			key = (INPUT_ConKey) { .type = INPUT_CON_KEY_BUTTON, .index = SDL_GAMEPAD_BUTTON_DPAD_DOWN };
			return pushed || INPUT_IsPressConKey(player, &key);
		case INPUT_BUTTON_LEFT:
			key = (INPUT_ConKey) { .type = INPUT_CON_KEY_AXIS, .index = 1 };
			pushed = INPUT_IsPressConKey(player, &key);
			key = (INPUT_ConKey) { .type = INPUT_CON_KEY_BUTTON, .index = SDL_GAMEPAD_BUTTON_DPAD_LEFT };
			return pushed || INPUT_IsPressConKey(player, &key);
		case INPUT_BUTTON_RIGHT:
			key = (INPUT_ConKey) { .type = INPUT_CON_KEY_AXIS, .index = 0 };
			pushed = INPUT_IsPressConKey(player, &key);
			key = (INPUT_ConKey) { .type = INPUT_CON_KEY_BUTTON, .index = SDL_GAMEPAD_BUTTON_DPAD_RIGHT };
			return pushed || INPUT_IsPressConKey(player, &key);
		case INPUT_BUTTON_A:
			key.type = INPUT_CON_KEY_BUTTON;
			if (type == INPUT_DEVICE_TYPE_NINTENDO) key.index = SDL_GAMEPAD_BUTTON_EAST;
			else key.index = SDL_GAMEPAD_BUTTON_SOUTH;
			return INPUT_IsPressConKey(player, &key);
		case INPUT_BUTTON_B:
			key.type = INPUT_CON_KEY_BUTTON;
			if (type == INPUT_DEVICE_TYPE_NINTENDO) key.index = SDL_GAMEPAD_BUTTON_SOUTH;
			else key.index = SDL_GAMEPAD_BUTTON_EAST;
			return INPUT_IsPressConKey(player, &key);
			break;
		case INPUT_BUTTON_C: key.type = INPUT_CON_KEY_BUTTON; key.index = SDL_GAMEPAD_BUTTON_WEST; return INPUT_IsPressConKey(player, &key);
		case INPUT_BUTTON_D: key.type = INPUT_CON_KEY_BUTTON; key.index = SDL_GAMEPAD_BUTTON_NORTH; return INPUT_IsPressConKey(player, &key);
		case INPUT_BUTTON_GIVE_UP: key.type = INPUT_CON_KEY_BUTTON; key.index = SDL_GAMEPAD_BUTTON_BACK; return INPUT_IsPressConKey(player, &key);
		case INPUT_BUTTON_PAUSE: key.type = INPUT_CON_KEY_BUTTON; key.index = SDL_GAMEPAD_BUTTON_START; return INPUT_IsPressConKey(player, &key);
		default: return 0;
		}
		break;
	#endif
...
int IsPushMenu(int32_t player, INPUT_Button button, INPUT_DeviceType type)
{
	#ifdef INPUT_ENABLE_GAME_CONTROLLER
	INPUT_ConKey key;
	int pushed;
	...
	case INPUT_DEVICE_TYPE_NINTENDO:
		...
		case INPUT_BUTTON_UP:
			key = (INPUT_ConKey) { .type = INPUT_CON_KEY_AXIS, .index = 3 };
			pushed = INPUT_IsPushConKey(player, &key);
			key = (INPUT_ConKey) { .type = INPUT_CON_KEY_BUTTON, .index = SDL_GAMEPAD_BUTTON_DPAD_UP };
			return pushed || INPUT_IsPushConKey(player, &key);
		case INPUT_BUTTON_DOWN:
			key = (INPUT_ConKey) { .type = INPUT_CON_KEY_AXIS, .index = 2 };
			pushed = INPUT_IsPushConKey(player, &key);
			key = (INPUT_ConKey) { .type = INPUT_CON_KEY_BUTTON, .index = SDL_GAMEPAD_BUTTON_DPAD_DOWN };
		...
		case INPUT_BUTTON_A:
			...
			return INPUT_IsPushConKey(player, &key);
		case INPUT_BUTTON_B:
			key.type = INPUT_CON_KEY_BUTTON;
			if (type == INPUT_DEVICE_TYPE_NINTENDO) key.index = SDL_GAMEPAD_BUTTON_SOUTH;
			else key.index = SDL_GAMEPAD_BUTTON_EAST;
			return INPUT_IsPushConKey(player, &key);
			break;
		case INPUT_BUTTON_C: key.type = INPUT_CON_KEY_BUTTON; key.index = SDL_GAMEPAD_BUTTON_WEST; return INPUT_IsPushConKey(player, &key);
		case INPUT_BUTTON_D: key.type = INPUT_CON_KEY_BUTTON; key.index = SDL_GAMEPAD_BUTTON_NORTH; return INPUT_IsPushConKey(player, &key);
		case INPUT_BUTTON_GIVE_UP: key.type = INPUT_CON_KEY_BUTTON; key.index = SDL_GAMEPAD_BUTTON_BACK; return INPUT_IsPushConKey(player, &key);
		case INPUT_BUTTON_PAUSE: key.type = INPUT_CON_KEY_BUTTON; key.index = SDL_GAMEPAD_BUTTON_START; return INPUT_IsPushConKey(player, &key);
		default: return 0;
		}
		break;
	#endif
...
#ifdef INPUT_ENABLE_GAME_CONTROLLER
int IsPressConTypeKey(INPUT_DeviceType type, INPUT_ConKey* key)
{
	#ifdef INPUT_ONLY_GAME_CONTROLLER_TYPE
	switch (INPUT_ONLY_GAME_CONTROLLER_TYPE)
	#else
	...
	for (int player = 0; player < INPUT_GetNumPlayerSlots(); player++) {
		if (INPUT_GetConType(player) == type && INPUT_IsPressConKey(player, key)) return 1;
	}
	return 0;
}

int IsPushConTypeKey(INPUT_DeviceType type, INPUT_ConKey* key)
{
	switch (type) {
	case INPUT_DEVICE_TYPE_XBOX:
	case INPUT_DEVICE_TYPE_PLAYSTATION:
	...
	for (int player = 0; player < INPUT_GetNumPlayerSlots(); player++) {
		if (INPUT_GetConType(player) == type && INPUT_IsPushConKey(player, key)) return 1;
	}
	return 0;
}
#endif

int IsPressPrompt(INPUT_Prompt prompt)
{
	switch (INPUT_GetLastDeviceType())
	{
	#ifdef ENABLE_LINUXGPIO
	case INPUT_DEVICE_TYPE_LINUX_GPIO:
		switch (input)
		{
		case INPUT_PROMPT_OK: return INPUT_IsPressGPIO(4);
		case INPUT_PROMPT_CANCEL: return INPUT_IsPressGPIO(5);
		case INPUT_PROMPT_RETRY: return INPUT_IsPressGPIO(6);
		default: return 0;
		}
	#endif
	...
	case INPUT_DEVICE_TYPE_KEYBOARD:
		switch (prompt)
		{
		case INPUT_PROMPT_OK: return INPUT_IsPressKey(SDL_GetScancodeFromKey(SDLK_RETURN, NULL));
		case INPUT_PROMPT_CANCEL: return INPUT_IsPressKey(SDL_GetScancodeFromKey(SDLK_BACKSPACE, NULL));
		case INPUT_PROMPT_RETRY: return INPUT_IsPressKey(SDL_GetScancodeFromKey(SDLK_DELETE, NULL));
		default: return 0;
		}
	#endif
	...
	case INPUT_DEVICE_TYPE_XBOX: {
		...
		switch (prompt)
		{
		case INPUT_PROMPT_OK: key.type = INPUT_CON_KEY_BUTTON; key.index = SDL_GAMEPAD_BUTTON_SOUTH; break;
		case INPUT_PROMPT_CANCEL: key.type = INPUT_CON_KEY_BUTTON; key.index = SDL_GAMEPAD_BUTTON_EAST; break;
		case INPUT_PROMPT_RETRY: key.type = INPUT_CON_KEY_BUTTON; key.index = SDL_GAMEPAD_BUTTON_WEST; break;
		default: return 0;
		}
		return IsPressConTypeKey(INPUT_DEVICE_TYPE_XBOX, &key);
	}
	...
	case INPUT_DEVICE_TYPE_PLAYSTATION: {
		...
		return IsPressConTypeKey(INPUT_DEVICE_TYPE_PLAYSTATION, &key);
	}
	...
	case INPUT_DEVICE_TYPE_NINTENDO: {
		...
		switch (prompt)
		{
		case INPUT_PROMPT_OK: key.type = INPUT_CON_KEY_BUTTON; key.index = SDL_GAMEPAD_BUTTON_EAST; break;
		case INPUT_PROMPT_CANCEL: key.type = INPUT_CON_KEY_BUTTON; key.index = SDL_GAMEPAD_BUTTON_SOUTH; break;
		case INPUT_PROMPT_RETRY: key.type = INPUT_CON_KEY_BUTTON; key.index = SDL_GAMEPAD_BUTTON_WEST; break;
		default: break;
		}
		return IsPressConTypeKey(INPUT_DEVICE_TYPE_NINTENDO, &key);
	}
	#endif
...
int IsPushPrompt(INPUT_Prompt prompt)
{
	switch (INPUT_GetLastDeviceType())
	{
	#ifdef ENABLE_LINUXGPIO
	case INPUT_DEVICE_TYPE_LINUX_GPIO:
		switch (input)
		{
		case INPUT_PROMPT_OK: return INPUT_IsPushGPIO(4);
		case INPUT_PROMPT_CANCEL: return INPUT_IsPushGPIO(5);
		case INPUT_PROMPT_RETRY: return INPUT_IsPushGPIO(6);
		default: return 0;
		}
	#endif
	...
	case INPUT_DEVICE_TYPE_KEYBOARD:
		switch (prompt)
		{
		case INPUT_PROMPT_OK: return INPUT_IsPushKey(SDL_GetScancodeFromKey(SDLK_RETURN, NULL));
		case INPUT_PROMPT_CANCEL: return INPUT_IsPushKey(SDL_GetScancodeFromKey(SDLK_BACKSPACE, NULL));
		case INPUT_PROMPT_RETRY: return INPUT_IsPushKey(SDL_GetScancodeFromKey(SDLK_DELETE, NULL));
		default: return 0;
		}
	#endif
	...
	case INPUT_DEVICE_TYPE_XBOX: {
		...
		return IsPushConTypeKey(INPUT_DEVICE_TYPE_XBOX, &key);
	}
	...
	case INPUT_DEVICE_TYPE_PLAYSTATION: {
		...
		return IsPushConTypeKey(INPUT_DEVICE_TYPE_PLAYSTATION, &key);
	}
	...
	case INPUT_DEVICE_TYPE_NINTENDO: {
		...
		return IsPushConTypeKey(INPUT_DEVICE_TYPE_NINTENDO, &key);
	}
	#endif
...
void testmenu(void) {
	...
	// GRAPHIC TEST
		...
		padRepeat(0);
		// ←
		if((mpc[0] == 1) || ((mpc[0] > tame1) && (mpc[0] % tame2 == 0)) || (getPressState(0, INPUT_BUTTON_C))) {
			if( getPressState(0, INPUT_BUTTON_LEFT) ) {
				param--;

				// 一時バッファを描画しない
				if( param == PLANE_TEMP ) param--;

				// 未使用プレーンを描画しない
				if( param == PLANE_UNUSED ) param--;

				// マイナスになっていたら79にする
				if( param < 0 ) param = PLANE_COUNT - 1;
			}
		}
		// →
		if((mpc[0] == 1) || ((mpc[0] > tame1) && (mpc[0] % tame2 == 0)) || (getPressState(0, INPUT_BUTTON_C))) {
			if( getPressState(0, INPUT_BUTTON_RIGHT) ) {
				param++;
				...
				// 79以上になっていたら0にする
				if( param >= PLANE_COUNT ) param = 0;
			}
		}

		if( !getPressState(0, INPUT_BUTTON_D) ) {
			printMenuButton(6, 29, INPUT_BUTTON_B, 0);
			printMenuButton(13, 29, INPUT_BUTTON_D, 0);
	...
	// RANKING ERASE
		...
		// A+Cで決定
		if( (!param) && (getPressState(0, INPUT_BUTTON_A)) && (getPressState(0, INPUT_BUTTON_C)) ) {
			PlaySE(WAVE_SE_KETTEI);
			RankingInit();
			RankingInit2();
			RankingInit3();
...
void LoadTitle() {
	if (!title_mov_f) {
		...
		LoadGraphic(PLANE_TITLE, "title");
		LoadGraphic(PLANE_LOGO, "logo");
	}
	else {
		// 動画
		// Movie
		// TODO: Implement animated image support (GIF, etc.) so this feature can work. Will require rearchitecting how "movies" are handled though, app code handling animation frames instead of game code
		LoadGraphic(PLANE_TITLE, "tmov");
		LoadGraphic(PLANE_LOGO, "logo");
	}
}
...
void loadGraphics(int32_t players) {
	...
	if(breakti) {
		...
		LoadGraphic(PLANE_BREAK3, "break3");
		LoadGraphic(PLANE_BREAK4, "break4");
		...
		LoadGraphic(PLANE_BREAK3, "break3_tap");
		LoadGraphic(PLANE_BREAK4, "break4_tap");
		LoadGraphic(PLANE_BREAK5, "break5_tap");
		LoadGraphic(PLANE_BREAK6, "break6_tap");
...
//	}
	...
	if(background == 2) {
		ExBltRect(PLANE_TITLE, 0, 0,0,0,320,240);

		for(i = 0; i < 3; i++)
			ExBlendBlt(PLANE_HEBOFLB0, (i % 3) * 120, 0, 256 - fldtr);
...
void loadBG(int32_t players){
	...
	if (load || getLastDrawRate() != getDrawRate()) {
		...
		LoadBackground(PLANE_BACK03, "back03");
		LoadBackground(PLANE_BACK04, "back04");
		LoadBackground(PLANE_BACK05, "back05");
		LoadBackground(PLANE_BACK06, "back06");
		LoadBackground(PLANE_BACK07, "back07");
		LoadBackground(PLANE_BACK08, "back08");
		LoadBackground(PLANE_BACK09, "back09");
		...
		for(i = 10; i < 72; i++) {
			...
			for(movframe = 0; movframe < framemax; movframe++) {
				...
				for(j = 0; j < 1 +players; j++) {
					if(background == 2) {
						ExBlendBltRect(PLANE_HEBOFLB0, (120 + 192 * j - 96 * players) + tmp1, 40 + tmp2, 0, 0, 80, 160, 256 - fldtr);
					}

					// グラデーション部分をなめらかに#1.60c7p9ex
					if(!top_frame){
						ExBlendBltRect(PLANE_HEBOSPR, (112 + 192 * j - 96 * players) + tmp1, tmp2, 101, 128, 100 - 20, 33, 256 - fldtr);
						for(k = 0; k < 20; k++) {
							tr = (256 - fldtr) * (19 - k) / 20;
							tr = 256 - tr;
							ExBlendBltRect(PLANE_HEBOSPR, (112 + 192 * j - 96 * players) + (80 + k) + tmp1, tmp2, 101 + (80 + k), 128, 1, 33, 256 - tr);
						}
					}
					else
					{
						ExBlendBltRect(PLANE_HEBOSPR, (97 + 192 * j - 96 * players) + tmp1, tmp2, 127, 423, 126, 35, 256 - fldtr);
					}
				}
			}
		}
...
void loadWaves(void) {
	...
	/* 効果音を読み込み */
	AUDIO_LoadWave(WAVE_SE_SHAKI, "res/se/shaki", false);
	AUDIO_LoadWave(WAVE_SE_KON, "res/se/kon", false);
	AUDIO_LoadWave(WAVE_SE_GON, "res/se/gon", false);
	AUDIO_LoadWave(WAVE_SE_KACHI, "res/se/kachi", false);
	AUDIO_LoadWave(WAVE_SE_ROTATE, "res/se/rotate", false);
	AUDIO_LoadWave(WAVE_SE_MOVE, "res/se/move", false);
	AUDIO_LoadWave(WAVE_SE_HOLD, "res/se/hold", false);
	AUDIO_LoadWave(WAVE_SE_TUMAGARI, "res/se/tumagari", false);
	AUDIO_LoadWave(WAVE_SE_GAMEOVER, "res/se/gameover", false);
	AUDIO_LoadWave(WAVE_SE_LVSTOP, "res/se/lvstop", false);
	AUDIO_LoadWave(WAVE_SE_KETTEI, "res/se/kettei", false);

	AUDIO_LoadWave(WAVE_SE_ERASE1, "res/se/erase1", false);
	AUDIO_LoadWave(WAVE_SE_ERASE2, "res/se/erase2", false);
	AUDIO_LoadWave(WAVE_SE_ERASE3, "res/se/erase3", false);
	AUDIO_LoadWave(WAVE_SE_ERASE4, "res/se/erase4", false);

	AUDIO_LoadWave(WAVE_SE_READY, "res/se/ready", false);
	AUDIO_LoadWave(WAVE_SE_GO, "res/se/go", false);

	AUDIO_LoadWave(WAVE_SE_APPLAUSE, "res/se/applause", false);
	AUDIO_LoadWave(WAVE_SE_CHEER, "res/se/cheer", false);
	AUDIO_LoadWave(WAVE_SE_LEVELUP, "res/se/levelup", false);

	AUDIO_LoadWave(WAVE_SE_UP, "res/se/up", false);
	AUDIO_LoadWave(WAVE_SE_BLOCK1, "res/se/block1", false);
	AUDIO_LoadWave(WAVE_SE_BLOCK2, "res/se/block2", false);
	AUDIO_LoadWave(WAVE_SE_BLOCK3, "res/se/block3", false);
	AUDIO_LoadWave(WAVE_SE_BLOCK4, "res/se/block4", false);
	AUDIO_LoadWave(WAVE_SE_BLOCK5, "res/se/block5", false);
	AUDIO_LoadWave(WAVE_SE_BLOCK6, "res/se/block6", false);
	AUDIO_LoadWave(WAVE_SE_BLOCK7, "res/se/block7", false);

	AUDIO_LoadWave(WAVE_SE_TTCLEAR, "res/se/ttclear", false);
	AUDIO_LoadWave(WAVE_SE_GM, "res/se/gm", false);
	AUDIO_LoadWave(WAVE_SE_RANKUP, "res/se/rankup", false);
	AUDIO_LoadWave(WAVE_SE_STAGECLEAR, "res/se/stageclear", false);
	AUDIO_LoadWave(WAVE_SE_HURRYUP, "res/se/hurryup", false);
	AUDIO_LoadWave(WAVE_SE_TIMEOVER, "res/se/timeover", false);
	AUDIO_LoadWave(WAVE_SE_TSPIN, "res/se/tspin", false);
	AUDIO_LoadWave(WAVE_SE_HANABI, "res/se/hanabi", false);
	AUDIO_LoadWave(WAVE_SE_MISSIONCLR, "res/se/missionclr", false);

	AUDIO_LoadWave(WAVE_SE_THUNDER, "res/se/thunder", false);

	AUDIO_LoadWave(WAVE_SE_WARNING, "res/se/warning", false);

	AUDIO_LoadWave(WAVE_SE_MEDAL, "res/se/medal", false);
	AUDIO_LoadWave(WAVE_SE_PINCH, "res/se/pinch", false);

	AUDIO_LoadWave(WAVE_SE_PLATINAERASE, "res/se/platinaerase", false);
	AUDIO_LoadWave(WAVE_SE_TIMEEXTEND, "res/se/timeextend", false);
	AUDIO_LoadWave(WAVE_SE_STGSTAR, "res/se/stgstar", false);
	AUDIO_LoadWave(WAVE_SE_ACE_SONIC_LOCK, "res/se/ace_sonic_lock", false);
	AUDIO_LoadWave(WAVE_SE_REGRET, "res/se/regret", false);
	AUDIO_LoadWave(WAVE_SE_COOL, "res/se/cool", false);

	AUDIO_LoadWave(WAVE_SE_TIMESTOP, "res/se/timestop", false);
	AUDIO_LoadWave(WAVE_SE_TSERASE, "res/se/tserase", false);
	//AUDIO_SetWaveLooping(WAVE_SE_PINCH, true);	//#1.60c7l6
}
...
void loadBGM(void) {
	...
	if (wavebgm & WAVE_BGM_SIMPLE) {
		if (load) {
			AUDIO_LoadMusic("res/bgm/bgm");
		}
		AUDIO_SetMusicVolume(bgmvolume);
		AUDIO_PlayMusic();
	}
	else if (load) {
		AUDIO_LoadWave(WAVE_BGM_BGM01, "res/bgm/bgm01", true);				// bgmlv 0 プレイ中（MASTER   0〜499）playwave(50)
		AUDIO_LoadWave(WAVE_BGM_BGM02, "res/bgm/bgm02", true);				// bgmlv 1 プレイ中（MASTER 500〜899）
		AUDIO_LoadWave(WAVE_BGM_BGM03, "res/bgm/bgm03", true);				// bgmlv 2 プレイ中（MASTER 900〜998、DEVIL 0〜499）
		AUDIO_LoadWave(WAVE_BGM_BGM04, "res/bgm/bgm04", true);				// bgmlv 3 プレイ中（DEVIL  500〜699）
		AUDIO_LoadWave(WAVE_BGM_BGM05, "res/bgm/bgm05", true);				// bgmlv 4 プレイ中（DEVIL  700〜999）
		AUDIO_LoadWave(WAVE_BGM_BGM06, "res/bgm/bgm06", true);				// bgmlv 5 プレイ中（DEVIL  1000以降）
		AUDIO_LoadWave(WAVE_BGM_ENDING, "res/bgm/ending", false);				// bgmlv 6 プレイ中（エンディング）
		AUDIO_LoadWave(WAVE_BGM_ENDING_B, "res/bgm/ending_b", false);			// bgmlv 7 プレイ中（BEGINNERエンディング）
		AUDIO_LoadWave(WAVE_BGM_TOMOYO, "res/bgm/tomoyo", true);				// bgmlv 8 プレイ中 通常（TOMOYO）
		AUDIO_LoadWave(WAVE_BGM_TOMOYO_EX, "res/bgm/tomoyo_ex", true);			// bgmlv 9 プレイ中 EXステージ（TOMOYO）
		AUDIO_LoadWave(WAVE_BGM_VSMODE, "res/bgm/vsmode", true);		        	// bgmlv 10 プレイ中（対戦モード）
		AUDIO_LoadWave(WAVE_BGM_TITLE, "res/bgm/title", true);		        	// bgmlv 11 タイトル
		AUDIO_LoadWave(WAVE_BGM_SELECT, "res/bgm/select", true);				// bgmlv 12 モードセレクト
		AUDIO_LoadWave(WAVE_BGM_NAMEENTRY, "res/bgm/nameentry", true);			// bgmlv 13 ネームエントリー
		AUDIO_LoadWave(WAVE_BGM_TOMOYO_EH, "res/bgm/tomoyo_eh", true);			// bgmlv 14 プレイ中 E-Heart（TOMOYO）
		AUDIO_LoadWave(WAVE_BGM_FEVER, "res/bgm/fever", true);				// bgmlv 15 FEVER発動中
		AUDIO_LoadWave(WAVE_BGM_MISSION_EX01, "res/bgm/mission_ex01", true);		// bgmlv 16 プレイ中 ミッションその1
		AUDIO_LoadWave(WAVE_BGM_MISSION_EX02, "res/bgm/mission_ex02", true);		// bgmlv 17 プレイ中 ミッションその2
		AUDIO_LoadWave(WAVE_BGM_MISSION_EX03, "res/bgm/mission_ex03", true);		// bgmlv 18 プレイ中 ミッションその3
		AUDIO_LoadWave(WAVE_BGM_TOMOYO_EH_FINAL, "res/bgm/tomoyo_eh_final", true);	// bgmlv 19 プレイ E-Heartラスト
	}
}
...
void spriteTime() {
	...
	if(IsPushKey(ssKey)) {			// スナップショット #1.60c
		...
		if( screenMode < 4 ) {
			SwapToSecondary(PLANE_TEMP);
			SaveBitmap(string[0], PLANE_TEMP, 0, 0, sw, sh);
			SwapToSecondary(PLANE_TEMP);
		} else {
			SaveBitmap(string[0], PLANE_TEMP, 0, 0, sw, sh);
		}
	...
	if(oncap && !(count % capi)) {	// 連続スナップ
		...
		SwapToSecondary(PLANE_TEMP);
		SaveBitmap(string[0], PLANE_TEMP, capx, capy, capw, caph);
		SwapToSecondary(PLANE_TEMP);
		capc++;
	}
	...
	if( screenMode >= 4 ) {
		...
		SwapToSecondary(PLANE_TEMP);		// セカンダリプレーンを、ダミーでプレーン9と入れ替える
		ExBltRectScaled(PLANE_TEMP, 0, 0, 0, 0, 320, 240, (65536 * ReSize) / 10 / getDrawRate(), (65536 * ReSize) / 10 / getDrawRate());
		SwapToSecondary(PLANE_TEMP);
		ExBltFast(PLANE_TEMP, 0, 0);
	}
	...
	switch (lastInputType = INPUT_GetLastDeviceType()) {
	#ifdef INPUT_ENABLE_LINUX_GPIO
	case INPUT_DEVICE_TYPE_LINUX_GPIO: {
		bool pushed = false;
		for (INPUT_Button button = 0; !pushed && button < NUMBTNS; button++) {
			pushed = INPUT_IsPushGPIO(button);
		}
		if (pushed) {
	...
	case INPUT_DEVICE_TYPE_KEYBOARD: {
		...
		int32_t pl;
		for (pl = 0; pl < 2; pl++) {
			for (INPUT_Button button = 0; !pushed && button < INPUT_BUTTON_COUNT; button++) {
				pushed = INPUT_IsPushKey(keyAssign[button + 10 * pl]);
			}
			if (pushed) break;
		}
		if (pushed) {
	...
	case INPUT_DEVICE_TYPE_JOYSTICK: {
		...
		for (pl = 0; pl < 2; pl++) {
			for (INPUT_Button button = 0; !pushed && button < INPUT_BUTTON_COUNT; button++) {
				pushed = INPUT_IsPushJoyKey(&joyKeyAssign[button + 10 * pl]);
			}
			if (pushed) break;
		}
		if (pushed) {
	...
	case INPUT_DEVICE_TYPE_PLAYSTATION:
	case INPUT_DEVICE_TYPE_NINTENDO: {
		int player = INPUT_GetLastActiveCon();
	...
	if (fourwayfilter != 3) {
		for (int32_t pl = 0; pl < 2; pl++) {
			if (
				...
				#ifdef INPUT_ENABLE_LINUX_GPIO
				if (pl == 0) {
					up = up || INPUT_IsPushGPIO(INPUT_BUTTON_UP);
					down = down || INPUT_IsPushGPIO(INPUT_BUTTON_DOWN);
					left = left || INPUT_IsPushGPIO(INPUT_BUTTON_LEFT);
					right = right || INPUT_IsPushGPIO(INPUT_BUTTON_RIGHT);
				}
				#endif

				#ifdef INPUT_ENABLE_KEYBOARD
				up = up || INPUT_IsPushKey(keyAssign[INPUT_BUTTON_UP + pl * 10]);
				down = down || INPUT_IsPushKey(keyAssign[INPUT_BUTTON_DOWN + pl * 10]);
				left = left || INPUT_IsPushKey(keyAssign[INPUT_BUTTON_LEFT + pl * 10]);
				right = right || INPUT_IsPushKey(keyAssign[INPUT_BUTTON_RIGHT + pl * 10]);
				#endif

				#ifdef INPUT_ENABLE_JOYSTICK
				up = up || INPUT_IsPushJoyKey(&joyKeyAssign[INPUT_BUTTON_UP + pl * 10]);
				down = down || INPUT_IsPushJoyKey(&joyKeyAssign[INPUT_BUTTON_DOWN + pl * 10]);
				left = left || INPUT_IsPushJoyKey(&joyKeyAssign[INPUT_BUTTON_LEFT + pl * 10]);
				right = right || INPUT_IsPushJoyKey(&joyKeyAssign[INPUT_BUTTON_RIGHT + pl * 10]);
				#endif

				#ifdef INPUT_ENABLE_GAME_CONTROLLER
				if (inmenu) {
					up = up || IsPushMenu(pl, INPUT_BUTTON_UP, INPUT_GetConType(pl));
					down = down || IsPushMenu(pl, INPUT_BUTTON_DOWN, INPUT_GetConType(pl));
					left = left || IsPushMenu(pl, INPUT_BUTTON_LEFT, INPUT_GetConType(pl));
					right = right || IsPushMenu(pl, INPUT_BUTTON_RIGHT, INPUT_GetConType(pl));
				}
				else {
					up = up || INPUT_IsPushConKey(pl, &conKeyAssign[INPUT_BUTTON_UP + 8 * pl]);
					down = down || INPUT_IsPushConKey(pl, &conKeyAssign[INPUT_BUTTON_DOWN + 8 * pl]);
					left = left || INPUT_IsPushConKey(pl, &conKeyAssign[INPUT_BUTTON_LEFT + 8 * pl]);
					right = right || INPUT_IsPushConKey(pl, &conKeyAssign[INPUT_BUTTON_RIGHT + 8 * pl]);
				}
				#endif
				...
				#ifdef INPUT_ENABLE_LINUX_GPIO
				if (pl == 0) {
					up = up || INPUT_IsPressGPIO(INPUT_BUTTON_UP);
					down = down || INPUT_IsPressGPIO(INPUT_BUTTON_DOWN);
					left = left || INPUT_IsPressGPIO(INPUT_BUTTON_LEFT);
					right = right || INPUT_IsPressGPIO(INPUT_BUTTON_RIGHT);
				}
				#endif

				#ifdef INPUT_ENABLE_KEYBOARD
				up = up || INPUT_IsPressKey(keyAssign[INPUT_BUTTON_UP + pl * 10]);
				down = down || INPUT_IsPressKey(keyAssign[INPUT_BUTTON_DOWN + pl * 10]);
				left = left || INPUT_IsPressKey(keyAssign[INPUT_BUTTON_LEFT + pl * 10]);
				right = right || INPUT_IsPressKey(keyAssign[INPUT_BUTTON_RIGHT + pl * 10]);
				#endif

				#ifdef INPUT_ENABLE_JOYSTICK
				up = up || INPUT_IsPressJoyKey(&joyKeyAssign[INPUT_BUTTON_UP + pl * 10]);
				down = down || INPUT_IsPressJoyKey(&joyKeyAssign[INPUT_BUTTON_DOWN + pl * 10]);
				left = left || INPUT_IsPressJoyKey(&joyKeyAssign[INPUT_BUTTON_LEFT + pl * 10]);
				right = right || INPUT_IsPressJoyKey(&joyKeyAssign[INPUT_BUTTON_RIGHT + pl * 10]);
				#endif

				#ifdef INPUT_ENABLE_GAME_CONTROLLER
				if (inmenu) {
					up = up || IsPressMenu(pl, INPUT_BUTTON_UP, INPUT_GetConType(pl));
					down = down || IsPressMenu(pl, INPUT_BUTTON_DOWN, INPUT_GetConType(pl));
					left = left || IsPressMenu(pl, INPUT_BUTTON_LEFT, INPUT_GetConType(pl));
					right = right || IsPressMenu(pl, INPUT_BUTTON_RIGHT, INPUT_GetConType(pl));
				}
				else {
					up = up || INPUT_IsPressConKey(pl, &conKeyAssign[INPUT_BUTTON_UP + 8 * pl]);
					down = down || INPUT_IsPressConKey(pl, &conKeyAssign[INPUT_BUTTON_DOWN + 8 * pl]);
					left = left || INPUT_IsPressConKey(pl, &conKeyAssign[INPUT_BUTTON_LEFT + 8 * pl]);
					right = right || INPUT_IsPressConKey(pl, &conKeyAssign[INPUT_BUTTON_RIGHT + 8 * pl]);
				}
				#endif
