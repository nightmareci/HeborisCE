...
void statSelectLevel(int32_t player) {
	...
	if(getPushState(player, INPUT_BUTTON_D)) {
		...
		printMenuButton(18 + 24 * player - 12 * maxPlay,26 , INPUT_BUTTON_C, player);
	...
	if((mpc[0] == 1) || ((mpc[0] > tame1) && (mpc[0] % tame2 == 0)) || (getPressState(player, INPUT_BUTTON_C))){
...
void PracticeStart(void){
	...
	// FPSを設定 #1.60c7n7
	if(max_fps != max_fps_2) MAIN_SetFPS(max_fps);
...
void PracticeDeath() {
	...
	// FPSを戻す #1.60c7n7
	if(MAIN_GetFPS() != max_fps_2) MAIN_SetFPS(max_fps_2);
	...
	printMenuButton(22 - 12 * maxPlay, 25, INPUT_BUTTON_C, -1);
