...
void RankingConvert(void) {
	...
	FILESYSTEM_ReadFile32("config/data/RANKING.SAV", saveBuf, 328, 0);
	for(i = 0; i < 40; i++) {
		temp = (i + 1) << 3;
...
void RankingProc2_1(void) {
	...
	if (flag == 0) {
		...
		if(getPushState(0, INPUT_BUTTON_A) || getPushState(1, INPUT_BUTTON_A) || getPushState(0, INPUT_BUTTON_B) || getPushState(1, INPUT_BUTTON_B)) {
			flag = -1;
		}


		if(getPushState(0, INPUT_BUTTON_UP) || getPushState(1, INPUT_BUTTON_UP) || getPushState(0, INPUT_BUTTON_DOWN) || getPushState(1, INPUT_BUTTON_DOWN)) {
			rankingmode = 1 - rankingmode;
			RankingCreate(category, rankingmode);
			count = 0;
		}

		if(getPushState(0, INPUT_BUTTON_LEFT) || getPushState(1, INPUT_BUTTON_LEFT)) {
			category = (category - 1);
			if(category < 0) category = 3;
			RankingCreate(category, rankingmode);
			count = 0;
		}
		if(getPushState(0, INPUT_BUTTON_RIGHT) || getPushState(1, INPUT_BUTTON_RIGHT)) {
...
int32_t RankingSave(void) {
	...
	FILESYSTEM_WriteFile32("config/data/RANKING.SAV", saveBuf, 328);
...
int32_t RankingLoad(void) {
	...
	FILESYSTEM_ReadFile32("config/data/RANKING.SAV", saveBuf, 4, 0);
