...
void RankingSave2() {
	...
	FILESYSTEM_WriteFile32("config/data/RANKING2.SAV", saveBuf, 1654);//1654=4+(5*15*2*11)
}
...
int32_t RankingLoad2() {
	...
	// ヘッダだけ読み込み
	SDL_memset(saveBuf, 0, 5000 * 4);
	FILESYSTEM_ReadFile32("config/data/RANKING2.SAV", saveBuf, 4, 0);
	...
	// 全部読み込み
	FILESYSTEM_ReadFile32("config/data/RANKING2.SAV", saveBuf, 1654, 0);
