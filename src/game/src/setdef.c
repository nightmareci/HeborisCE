...
typedef struct {
	...
	INPUT_JoyKey	joyKeyAssign[10 * 2];
	#endif
	...
	VIDEO_ScreenModeFlag	screenMode;
	int32_t		screenIndex;
	int32_t		soundbuffer;
	int32_t		nextblock;
	uint32_t	cfgversion;
...
static const SConfig DefaultConfig = {
	...
	#ifdef INPUT_ENABLE_JOYSTICK
	.joyKeyAssign = INPUT_JOY_KEY_ASSIGN_DEFAULT,		//ジョイスティックボタン割り当て
	#endif
...
void SetDefaultConfig()
{
	...
	for (int32_t pl = 0; pl < 2; pl++) {
		for (int32_t key = 0; key < 10; key++) {
			...
			const INPUT_JoyKey *pljoy = &DefaultConfig.joyKeyAssign[pl * 10 + key];
	...
	for (int32_t pl = 0; pl < 2; pl++) {
		...
		for (int32_t key = 0; key < 8; key++) {
			const INPUT_ConKey *plcon = &DefaultConfig.conKeyAssign[pl * 8 + key];
