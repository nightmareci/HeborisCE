...
void StopAllSE(void) {
	...
	for(i = 0; i < WAVE_BGM_START; i++) {
		AUDIO_StopWave(i);
		AUDIO_SetWaveVolume(i, sevolume);	// #1.60c7o8
	}
}
...
void StopAllBGM(void) {
	...
	for(i = WAVE_BGM_START; i < WAVE_BGM_START + WAVE_BGM_COUNT; i++) {
		AUDIO_StopWave(i);
		AUDIO_SetWaveVolume(i, bgmvolume);	// #1.60c7o8
	}
}
...
void bgmFadeout(void) {
	...
	if(wavebgm & WAVE_BGM_SIMPLE) return;
	...
	for( player = 0; player <= maxPlay; player++ ) {
		...
		if(fadelv[player] != 0) {
			...
			AUDIO_SetWaveVolume(WAVE_BGM_START+bgmlv, (int)(bgmvolume * ((10000 - fadelv[player]) / 10000.0f)));
		}
	}
}
...
void changeBGM(int32_t player) {
	...
	fadelv[player] = 0;
	//if((!(wavebgm & WAVE_BGM_SIMPLE)) && (gameMode[0] != 8)) bgmlv = 3;
}
