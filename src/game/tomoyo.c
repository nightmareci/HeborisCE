...
void statTomoyoNextStage(int32_t player) {
	...
	// STAGE XX
	printFont(16-((((stage[player] >= 45) && (stage[player] <= 67)) || (stage[player] >= 100)))-(((stage[player] >= 36) && (stage[player] <= 44))) + 24 * player - 12 * maxPlay, 6, "STAGE", 4);
	...
	printFont(22-(((stage[player] >= 45) && (stage[player] <= 67)) || (stage[player] >= 100))-((stage[player] >= 36) && (stage[player] <= 44)) + 24 * player - 12 * maxPlay, 6, string[0], 0);
	...
	if(!fpbas_mode[player]){
		...
		printSMALLFont( (15 + 24 * player - 12 * maxPlay)*8 + 8, 11*8 + 1, string[0], ((count % 4 / 2) * digitc[rotspl[player]])*(ext != 0) );
	}
	...
	if(statusc[player * 10 + 0] >= 300) {
		if(start_stage[player] >= 27) {
			...
			if(clearp[player] < 90) {
				...
				if (gametime[player] < timelimit_tomoyo) {
					// 5分以内なら最後まで
					laststage[player] = 26;
					//EX7行きが決まれば流れ星
...
void statTomoyoEditor(int32_t player) {
	...
	if( statusc[player * 10 + 1] == 0 ) {
		printFont(15 + 24 * player - 12 * maxPlay, 8 + statusc[player * 10] +((statusc[player * 10] >= 13)) , "b", 1);
	...
	/* ■フィールド画面■ */
		...
		// Aボタンで配置
		if( getPressState(player, INPUT_BUTTON_A) && !statusc[player * 10 + 3] ) {
		...
		if( !getPressState(player, INPUT_BUTTON_A) ) {
			statusc[player * 10 + 3] = 0;
		}
...
void statTomoyoSelect(int32_t player) {
	...
	// HOLDボタンでnextcを0に戻す
	if( getPushState(player, INPUT_BUTTON_D) ) {
		AUDIO_PlayWave(WAVE_SE_KACHI);
...
void statTomoyoResult(int32_t player) {
	...
	if( (status[1 - player] == 0) && (!AUDIO_IsWavePlaying(WAVE_BGM_NAMEENTRY)) && !(wavebgm & WAVE_BGM_SIMPLE) ) AUDIO_PlayWave(WAVE_BGM_NAMEENTRY);
	...
	if( (!playback) && (!ending[player]) ) {
		printSMALLFont(120 + 192 * player - 96 * maxPlay, 144, "CONTINUE", (statusc[player * 10] == 0) * fontc[rotspl[player]]);
		printSMALLFont(120 + 192 * player - 96 * maxPlay, 154, "END", (statusc[player * 10] == 1) * fontc[rotspl[player]]);
	...
	// ↑↓
	if(getPushState(player, INPUT_BUTTON_UP) || getPushState(player, INPUT_BUTTON_DOWN)) {
		if(!ending[player]) {
			AUDIO_PlayWave(WAVE_SE_MOVE);
	...
	// Aボタン
	if( (getPushState(player, INPUT_BUTTON_A)) || (statusc[player * 10 + 1] >= 10 * 60) ) {
		StopAllBGM();
...
void statTomoyoSelect_FP(int32_t player) {
	...
	if(statusc[player * 10] == 0){	//最初
		...
		printFont(15 + 24 * player - 12 * maxPlay, 10 + ( statusc[player * 10 + 2] * 2), "b", fontc[rotspl[player]]);
	...
	// Aボタン
	if( (getPushState(player, INPUT_BUTTON_A)) || ((statusc[player * 10] != 0) && (statusc[player * 10 + 1] > 600)) ) {
		AUDIO_PlayWave(WAVE_SE_KETTEI);
		if( statusc[player * 10] == 0 ) {
			// 最初
			stage[player] = start_stage[player];
	...
	if(( statusc[player * 10] == 0 ) && (getPushState(player, INPUT_BUTTON_B))) {		// Bボタンでモード選択画面に戻る
		sp[player] = 1;
		AUDIO_PlayWave(WAVE_SE_MOVE);
		randommode[player] = 1;
...
void loadTomoyoStage(int32_t player, int32_t number) {
	...
	FILESYSTEM_ReadFile32(string[0], saveBuf, 450, 0);
...
void saveTomoyoStage(int32_t player, int32_t number) {
	...
	// 保存する
	SDL_snprintf(string[0], STRING_LENGTH, "config/stage/stage%02d.sav", number);
	FILESYSTEM_WriteFile32(string[0], saveBuf, 450);
}
