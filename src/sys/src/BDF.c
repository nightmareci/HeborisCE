...
static bool BDF_ParseChar(BDF_Font* font, uint32_t encoding, int width, char** line, char* end, int rshift)
{
	if (HASHTABLE_HasValue(font->glyphs, encoding)) {
		return true;
	}

	BDF_Glyph* glyph = SDL_malloc(sizeof(BDF_Glyph));
	if (!glyph) {
		return ERROR_Set("Failed allocating glyph data to parse a BDF character");
	}
	glyph->width = width;

	glyph->pixels = SDL_malloc(sizeof(uint32_t) * font->size);
	if (!glyph->pixels) {
		ERROR_Set("Failed to allocate memory for a character's pixels of a font.");
		SDL_free(glyph);
		return false;
	}
	...
	if (HASHTABLE_SetValue(font->glyphs, encoding, glyph, BDF_DestroyGlyph, NULL)) {
		return true;
	}
	else {
...
bool BDF_AddFont(BDF_Font* font, SDL_IOStream* src)
{
	...
	if (SDL_ReadIO(src, srcData, srcSize) == 0) {
		ERROR_Set("Failed to read the font data: %s", SDL_GetError());
		SDL_free(srcData);
		return false;
	}
	...
	while (BDF_GetLine(buffer, sizeof(buffer), &line, end) != NULL) {
		if (SDL_strstr(buffer, "SIZE") == buffer) {
			if (foundSize) {
				SDL_free(srcData);
				return ERROR_Set("Erroneous extra SIZE line encountered.");
			}
			p = buffer;
			while (*p != ' ' && *p != '\t') p++;
			while (*p == ' ' || *p == '\t') p++;
			...
			if (fontSize <= 0 || fontSize > INT_MAX) {
				SDL_free(srcData);
				return ERROR_Set("Font size of %ld is invalid, must be between 0 and %d.", fontSize, INT_MAX);
			}
			if (font->size == 0) {
			...
			if (!foundSize) {
				SDL_free(srcData);
				return ERROR_Set("SIZE line not found before a STARTCHAR line.");
			}
			foundEncoding = false;
			foundWidth = false;
		}
			...
			if (!foundSize) {
				SDL_free(srcData);
				return ERROR_Set("SIZE line not found before an ENCODING line.");
			}
			p = buffer;
			while (*p != ' ' && *p != '\t') p++;
			while (*p == ' ' || *p == '\t') p++;
			...
			if (encoding < 0) {
				SDL_free(srcData);
				return ERROR_Set("Character encoding of %ld is invalid, must be nonnegative.", encoding);
			}
			foundEncoding = true;
		}
			...
			if (!foundSize) {
				SDL_free(srcData);
				return ERROR_Set("SIZE line not found before a DWIDTH line.");
			}
			p = buffer;
			while (*p != ' ' && *p != '\t') p++;
			while (*p == ' ' || *p == '\t') p++;
			...
			if (width <= 0 || width > INT_MAX) {
				SDL_free(srcData);
				return ERROR_Set("Character width of %ld is invalid, must be between 0 and %d.", width, INT_MAX);
			}
			foundWidth = true;
		}
			...
			if (!foundSize || !foundEncoding || !foundWidth) {
				SDL_free(srcData);
				return ERROR_Set("BITMAP line encountered before all of SIZE, ENCODING, and DWIDTH lines have been found.");
			}
			for (s = 8; s < width; s += 8) {}
			rshift = s - (int)width;
			if (!BDF_ParseChar(font, (Uint32)encoding, (int)width, &line, end, rshift)) {
				SDL_free(srcData);
				return ERROR_Set("Failed to parse a font character: %s", SDL_GetError());
			}
		}
	}
...
BDF_Font* BDF_OpenFont(SDL_IOStream* src)
{
	...
	if (!font) {
		ERROR_Set("Failed to allocate memory for the font object.");
		return NULL;
	}

	font->size = 0;

	font->glyphs = HASHTABLE_Create(0x100);
	if (!font->glyphs) {
		SDL_free(font);
		return NULL;
	}
...
int BDF_GetTextWidth(BDF_Font* font, const char* text)
{
	...
	if (font == NULL) {
		return ERROR_Set("Parameter \"font\" must not be NULL.");
	}
	else if (text == NULL) {
		return ERROR_Set("Parameter \"text\" must not be NULL.");
	}
	...
	for (p = BDF_NextUTF8Encoding((const Uint8*)text, &encoding); p != NULL && encoding != '\0' && encoding != '\n' && encoding != '\r'; p = BDF_NextUTF8Encoding(p, &encoding)) {
		...
		if (!HASHTABLE_GetValue(font->glyphs, encoding, (void**)&glyph)) {
			return ERROR_Set("The text contains a character not in the font, so width cannot be calculated.");
		}
		if (w > INT_MAX - glyph->width) {
			return ERROR_Set("The calculated width of the text overflowed (maximum possible width is %d).", INT_MAX);
		}
	...
	if (p != NULL) {
		...
		return ERROR_Set("Invalid data found in the text. The text must be valid UTF-8 data.");
	}
}
	...
	for (p = BDF_NextUTF8Encoding((const Uint8*)text, &encoding); p != NULL && encoding != '\0'; p = BDF_NextUTF8Encoding(p, &encoding)) {
		...
		if (encoding == '\n' || encoding == '\r') {
			...
			if (temp_h > INT_MAX - font->size) {
				return ERROR_Set("The calculated height of the text overflowed (maximum possible height is %d).", INT_MAX);
			}
			temp_h += font->size;
			if (line_w > temp_w) {
				temp_w = line_w;
		...
		if (!HASHTABLE_GetValue(font->glyphs, encoding, (void**)&glyph)) {
			return ERROR_Set("The text contains a character not in the font, so dimensions cannot be calculated.");
		}
...
bool BDF_PutText(BDF_Font* font, int dx, int dy, float subx, float suby, void* dst, int dw, int dh, const char* txt, SDL_Color fg, BDF_PutPixelCallback put_pixel)
{
	...
	for (p = BDF_NextUTF8Encoding((const uint8_t*)txt, &encoding); p != NULL && encoding != '\0'; p = BDF_NextUTF8Encoding(p, &encoding)) {
		...
		if (!HASHTABLE_GetValue(font->glyphs, encoding, (void**)&glyph)) {
			return ERROR_Set("The text contains a character not in the font (U+%04" SDL_PRIX32 "), so it cannot be drawn.", encoding);
		}
...
bool BDF_PutTextSurface(BDF_Font* font, int dx, int dy, float subx, float suby, SDL_Surface* dst, const char* txt, SDL_Color fg)
{
	return BDF_PutText(font, dx, dy, subx, suby, dst, dst->w, dst->h, txt, fg, (BDF_PutPixelCallback)BDF_PutPixelSurface);
}

bool BDF_PutTextRenderer(BDF_Font* font, int dx, int dy, float subx, float suby, SDL_Renderer* dst, const char* txt, SDL_Color fg)
{
	int dw, dh;
...
{
	int w, h;
	if (BDF_GetTextDimensions(font, text, &w, &h) < 0) {
		return NULL;
	}
	...
	default:
		ERROR_Set("Invalid depth of %d, only 16 or 32 are supported.", depth);
		return NULL;
	}
	...
	if (!BDF_PutTextSurface(font, 0, 0, 0.0f, 0.0f, surface, text, fg)) {
		SDL_DestroySurface(surface);
		return NULL;
	}
...
{
	SDL_Surface* surface = BDF_CreateTextSurface(font, text, fg, depth);
	if (!surface) {
		return NULL;
	}
